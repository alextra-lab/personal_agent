...
| **2b — Compose unification** | Merge `docker-compose.yml` + `docker-compose.cloud.yml` via compose profiles. Containerize laptop gateway with `develop.watch`. **Each location runs its own self-contained datastores** (peer-deployment model — confirmed 2026-05-08). Plan must include a sync procedure for Alembic migrations, ES index templates, and Kibana saved objects between laptop and VPS stacks. | Lands on top of 2a. Confidence that endpoints resolve correctly inside containers comes from 2a's probe logic. |
