...
| **2b — Compose unification** | Merge `docker-compose.yml` + `docker-compose.cloud.yml` via compose profiles. Containerize laptop gateway with `develop.watch`. **Each location runs its own self-contained datastores by default** (peer-deployment model — confirmed 2026-05-08). Plan deliverables: (i) compose merge with `profiles: [cloud]` markers; (ii) sync procedure for Alembic migrations + ES index templates + Kibana saved objects between laptop and VPS stacks; (iii) **opt-in tunnel mode** — `.env.local-to-vps` override + `make tunnel-data` (ssh -L for postgres/neo4j/es/redis) + startup safety guard requiring `FORCE_LIVE_DATA=1` when tunnel mode is detected with write-capable roles. | Lands on top of 2a. Confidence that endpoints resolve correctly inside containers comes from 2a's probe logic. |
