...
- **Out of scope (Phase 2, gated → follow-on ADR):** building any fix — Mem0-style write ops, temporal-KG validity windows, retrieval/reranker changes, or a structural evolution of the **Core** graph for narrative/episodic recall *(the Markdown/LLM-wiki **substrate** this once listed is ruled out — ADR-0098 settled it; only the contradiction-linting idea was kept, on Core)*. Which of these we build is **selected by the gates in §D5**, not decided here.
...
3. **Architecture gate (H5 / H6)** — keyed on the residual-failure count after gates 1–2 are addressable in principle. Fires if facts are present and retrievable but the *structure* cannot represent what's asked (narrative/episodic recall). → route to a **Phase-2 architecture ADR** that **evolves the Core graph structure** to represent the missing relation (episodic/narrative — e.g. the spatio-temporal episode model, FRE-467) and **A/Bs it on the same probe set**. **The markdown / LLM-wiki *substrate* is ruled out** — ADR-0098 (Accepted 2026-06-27) adopted the wiki's contradiction-linting *idea* on the Core entity spine and rejected the markdown substrate on FRE-635 evidence; gate 3 evolves Core structure, it does not revert to a parallel note store. (The A/B is Phase-2, not Phase-1.)

Gates are *not mutually exclusive* — Phase-1 reports the share of failures attributable to each and routes to the dominant one(s). The Phase-2 architecture decision is a **follow-on ADR**, gated on these results — and **part of it has already landed**: the memory rebuild **ADR-0097 / ADR-0098** (Accepted 2026-06-27) is the write-path + substrate Phase-2 work this ADR anticipated, and it has **pre-decided the substrate question** (living Claims on one Neo4j Core graph; markdown/LLM-wiki substrate rejected, its contradiction-linting idea absorbed into 0098 D2). What remains genuinely open for gate 3 is narrower: *if a structural/narrative-recall gap survives 0098's Claims model*, the follow-on evolves **Core structure** (episodic/narrative relations), tested against the same evidence — not a parallel markdown store.
...
- The Phase-2 architecture choice (KG-write-fix vs retrieval-fix vs Core-structure evolution for narrative/episodic) is **pre-registered and evidence-gated**, immune to narrative drift. *(The substrate question is already settled by ADR-0098 — markdown/LLM-wiki ruled out, its idea absorbed into Core D2.)*
