...
**Status:** Implemented — 2026-06-07 (FRE-509 Worker CSP · FRE-510 iframe sandbox flip · FRE-511 sanitizer retirement · FRE-512 envelope-integrity verification; prod-verified `make verify-envelope` exit 0, 12/12 directives exact). **Addendum A (FRE-525, 2026-06-08):** realizes the D2a curated toolkit + the "toolkit contents and versioning policy" open decision — see [Addendum A](#addendum-a--curated-toolkit-realization-fre-525) below.
...
## Addendum A — Curated Toolkit Realization (FRE-525)

**Added:** 2026-06-08 · **Linear:** FRE-525 · **Realizes:** D2a (curated self-hosted toolkit) and the "Open decisions → *Toolkit contents and versioning policy*" line. **Builds on:** FRE-509 (the `/lib/` slot + MIME/`nosniff`/route controls that make a host-source allowlist mean "vetted libraries only"). **Supersedes:** FRE-498 (its mermaid-CDN measurement question is folded into A1 and answered by the instrument fix). **Reframes:** FRE-511's generation prompt. **Scope addition (owner, 2026-06-08):** export-to-standalone (portable artifacts) is in scope here, not deferred.

ADR-0089 sealed the box and proved the walls (`make verify-envelope`, 12/12 directives). But the shelf inside is **empty and unadvertised**: zero libraries are hosted on `/lib/`, and the generation prompt (`_HTML_GENERATION_SYSTEM_PROMPT`, `tools/artifact_tools.py:881`) still tells the model *"no external resources … everything must be inline,"* naming Tailwind/Alpine/jQuery/fonts as things that "silently fail" — and never points at `/lib/`. The sealed box is **safe but unequipped**: for math, real charts, 3-D, code, book-grade typography, or pagination the model must hand-roll inline (token-expensive, quality-limited) or reach for a CDN that `connect-src 'none'` + the host-allowlist **silently blocks at view time** (the artifact commits intact but renders broken). This addendum makes the sealed box genuinely *useful*, and makes its output *portable*. It changes **no security invariant** of ADR-0089 — every library rides the same D2/D3 envelope.

### A1 — The demand signal is not yet measurable; the instrument is blind (Phase-1 finding)

The measure-first methodology (FRE-433/434) was applied and hit an honest wall worth recording, because it shapes the decision:

- **The meter cannot see the dominant reach.** The only demand instrument is the `cdn_count` gate-label, computed by `_CDN_LINK_RE` (`artifact_tools.py:792`), which matches **only `<link href="http…">`** (stylesheets/fonts). It does **not** match `<script src="https://…">` — exactly the mermaid/charting/3-D CDN-reach vector FRE-498 set out to count. The most common reach pattern is structurally invisible to today's telemetry.
- **The corpus is too thin and not indexed.** Only **4** `artifact_gate_decision` events exist (FRE-506/509 just shipped), all `cdn_count=0`. Artifact *bytes* are not full-text-indexed in Elasticsearch (long-HTML field truncation) and authoritatively live in R2 + Postgres, so historical content cannot be reliably mined from ES. A lower-bound full-text scan of the captures index returns `mermaid`≈20 (the supported SVG path is used), `chart.js`≈2, and ≈0 for katex/three/tailwind-cdn/d3/alpine/jquery.

**Consequence (decision-shaping):** the initial set is **reasoned, not data-derived** — chosen from capability-envelope coverage + frontier-lab parity + the D2a constraints (byte cost · eval-free · classic-script · permissive license · gap-closed). The measure-first loop is honored by **repairing the instrument** so the set is tunable from here, not by pretending the corpus exists. The instrument fix (ticket #1) extends the non-load-bearing label to count + classify external `<script src>` reaches, distinguishing **host-allowed** (`…/lib/…`, a satisfied need) from **host-blocked** (`cdn.*`, an *unmet capability demand* — the real signal). That closes FRE-498's question deterministically going forward.

### A2 — The curated set: complete and well-rounded, chosen once

The brief is a **complete, well-rounded toolkit** for the dynamic, interactive, compelling teaching and authoring artifacts ADR-0089 set out to enable — not a minimal shelf that forces a revisit, and not a kitchen sink. The decisive split: most *dynamism* (sliders, simulations, explorables, animation, tabs/filters) needs **no library** — inline JS + Canvas/WebGL/SVG is already unlocked by ADR-0089. Libraries (and fonts) are warranted only where the model genuinely **cannot hand-roll** the capability well.

**Frontier-lab parity (verified 2026-06-08).** All three majors expose the same *capability envelope* in-sandbox — charts, math, 3-D, diagrams, code — but deliver it two ways. **Claude Artifacts** ship a client-side JS shelf (React artifacts curate Recharts / three.js / d3 / lucide; HTML artifacts may load external scripts but **only from a single allowlisted CDN, `cdnjs.cloudflare.com`** — the egress-sealed-shelf model our `/lib/` mirrors). **OpenAI** and **Google** lean on a **Python code-execution sandbox** (Pyodide / code-interpreter: numpy/pandas/matplotlib) for the data/chart half, with a code-preview canvas for the web half; OpenAI's **Apps SDK** widgets talk to the host over **`window.openai` postMessage JSON-RPC** — independent convergence on ADR-0089 D4a's parent-broker bridge. We are an HTML-artifact model with **no Python sandbox and sealed egress**, so the right parity target is "Claude HTML artifacts using allowlisted libs," and `/lib/` is its **self-hosted, Access-fronted** form. (Sources in References.)

| Capability | Library | License | In/Out | Rationale (D2a lens) |
|---|---|---|---|---|
| Math / formula typesetting | **KaTeX** | MIT | **In** | The clearest "model can't hand-roll" gap. Small, **no `eval`**, no network. Classic `<script>` + `katex.min.css` + a **co-hosted `fonts/` dir** (the CSS resolves `url(fonts/…)` relatively — A3). |
| Charts / data visualization | **Chart.js** (v4) | MIT | **In** | Recurring gap (today the model hand-rolls SVG/canvas chart math). Clean UMD global `Chart`, **no `eval`**, canvas. Owner-selected over uPlot/D3 for first-try correctness. |
| 3-D / spatial / physics | **three.js** | MIT | **In** | Headline "compelling" capability (molecules, geometry, physics sims) — unmaintainable by hand. **One-time hosting task:** modern three ships ESM + CJS but **no browser-global IIFE build** usable from a bare `<script src>` (its CJS is Node-style, not a browser global); D2a requires classic scripts → prepare + host a bundled IIFE/global build (A3). Including it now is the "don't revisit" call. |
| Code / syntax highlighting | **highlight.js** | BSD-3 | **In** | Coding-tutor surface (a live owner learning thread). Global `hljs`, **no `eval`**, classic `<script>` + theme `.css`; `hljs.highlightAll()`. |
| Typography (book-grade prose) | **Curated fonts** | SIL OFL | **In** | The biggest lever for publishing-quality prose; the design system ships **system fonts only** today. A serif *text* face + a *display* face + mono, self-hosted on `/lib/`. **No CSP change** (`font-src` already allows the host + `data:`). OFL-only keeps self-hosting clean. |
| Book / print layout (pagination) | **paged.js** | MIT | **In** (license permits) | The one publishing-specific lib that closes a hand-roll-impossible gap: running heads, page numbers, **footnote placement**, `@page` boxes. MIT → owner's "include if licensing permits" is satisfied. **Hosting gate:** if it proves to need `eval` under our CSP (no `'unsafe-eval'`), it falls back to deferred. |
| Diagrams (portable) | **Mermaid** | — | **In — existing path** | Already server-rendered to inline SVG (FRE-396); the *portability* lane. **Not** a client `/lib/` lib — the SVG path is strictly better for static diagrams and travels with an exported file. |
| Sliders · simulations · explorables · animation | **native** | — | **In — no library** | Inline JS + Canvas/WebGL/SVG/CSS, unlocked by ADR-0089. No `/lib/` entry. |

Every `/lib/` library is **permissively licensed** (MIT / BSD / OFL — clean to self-host), **eval-free** (consistent with the CSP omitting `'unsafe-eval'`; paged.js gated on confirmation), and **classic-script-deliverable** (D2a). All ride the existing envelope unchanged: `script-src …'unsafe-inline'` admits the `/lib/` `<script src>` + inline init; `style-src …` admits the `<link>` lib CSS + inline styles; `font-src … data:` admits KaTeX + the curated fonts; `connect-src 'none'` stays — **a corollary the guidance must teach: 3-D textures/models and chart data must be inlined (data-URI / inline JS), never fetched.**

**Deferred — explicitly, so the line is durable** (revisited only on measured demand from the repaired meter): **Tailwind** (overlaps the prompt's hand-rolled design system; large); **D3** (Chart.js + native JS + three.js cover the viz envelope); **Tone.js / audio** (native Web Audio is the floor; revisit if music-teaching becomes a focus); **data-parsing** (papaparse/xlsx — our model *inlines* parsed data, there is no file-upload-into-artifact path, so a parser earns less); **mapping** (Leaflet et al. need network tiles the sandbox blocks). None are rejected forever; all are out of the complete-toolkit v1.
...
### A4 — Teaching the model: prompt reframe + runtime guidance (Phase 3)

- **Generation-prompt reframe** (`_HTML_GENERATION_SYSTEM_PROMPT`). Replace the blanket *"no external resources / everything inline"* with the **sanctioned toolkit**: the exact `/lib/` `<script src>` / `<link>` snippets, when to reach for each (KaTeX→math, Chart.js→data viz, three.js→3-D, highlight.js→code, the fonts→prose, paged.js→book/print layout, mermaid-SVG→static diagrams that should travel), the **native** typography recipes (drop caps via `::first-letter`, `text-align: justify` + `hyphens: auto`, `text-wrap: balance`, `font-feature-settings`, CSS `column-count`/`@page`), and the **still-true** constraints (no *arbitrary* CDN, no network/`fetch`, no storage; inline your own data/textures). This is the **runtime delivery surface** — the generation sub-agent is driven by this system prompt (`artifact_tools.py:1299`); it does not load SKILL.md at runtime.
- **`docs/skills/artifact-design.md`** (new project skill, self-describing pattern, sibling to `docs/skills/mermaid-diagrams.md`). The **maintainable source-of-truth** the prompt is distilled from + the future target for a skill-loading mechanism: per-library recipes + gotchas (KaTeX CSS + delimiters; Chart.js `<canvas>`/data-inlining; three.js scene/renderer boot + "textures must be data-URI"; highlight.js `highlightAll` + theme; paged.js page-box setup; the OFL font stack), the design-system conventions, the portability decision (static-SVG-travels vs interactive-viewed-on-origin vs **exported-standalone**), and the standing rule **"never bake secrets into an artifact"** (D4).
- **Compose-vs-new — decided: new.** The existing `frontend-design` skill is a **Claude Code harness** skill (guidance for the *developer* agent). Its consumer is **not** the runtime artifact-generation sub-agent. The two surfaces do not compose; `artifact-design.md` is a distinct runtime-guidance artifact.

### A5 — Exportability & portability (in scope, owner-directed)

ADR-0089 D7/FRE-397 named a portability axis; this addendum makes it a real capability. Two distinct "exports" exist and must not be conflated:

1. **Export the artifact *document*** — gated by **self-containment**. An inline-only artifact already travels; a **toolkit-using** artifact references Access-gated `/lib/` and is otherwise **view-on-origin-only**. Resolution (**in scope**): a server-side **export-to-standalone** endpoint (it can read `/lib/` and R2 trusted, server-side) offering **two modes**, both driven by a per-asset **export substitution map** maintained alongside the toolkit:
   - **Inline (default, offline-portable):** fetch the referenced **pinned `/lib/` assets + fonts** and **inline** them (scripts inline, CSS inline, fonts/images as `data:` URIs) into one **self-contained HTML file** (`Content-Disposition: attachment`) that renders **offline, anywhere**. *Why pinned + immutable `/lib/` matters — the inliner trusts the exact bytes.*
   - **Substitution-map (opt-in, lean/online):** **rewrite** each `…/lib/<name>@<version>` reference to the **same version on a public CDN** (e.g. jsDelivr/cdnjs), emitting **Subresource Integrity** (`integrity=` + `crossorigin`) computed from our pinned bytes — so the exported file is tiny, human-editable, and **byte-verified** against the CDN, yet portable wherever there is internet (the Claude-style publish/remix path). **SRI on a cross-origin load requires the CDN to return a permissive `Access-Control-Allow-Origin`** — without CORS the browser blocks the fetch *before* the hash is even checked — so each CDN entry must be **CORS-verified** at map-build time (jsDelivr/cdnjs serve `Access-Control-Allow-Origin: *`; this is a verification gate, not an assumption).
   - **Per-asset rule (robust hybrid):** substitute *only where a faithful public twin exists at the pinned version, SRI is computed, **and** CORS is verified present*; **otherwise fall back to inline.** KaTeX / Chart.js / highlight.js / paged.js map cleanly; **three.js inlines** (our `/lib/` copy is a self-bundled *classic* build with no exact public classic twin — A3) and **fonts inline** (cleaner than a UA-varying font-CDN). The map records `{public-CDN URL, SRI hash, CORS-verified, classic|module, fallback=inline}` per `/lib/` entry, and doubles as the PWA-convergence version reference (A6). Mermaid's server-SVG already inlines, so diagram artifacts export cleanly in either mode.
   - **Honest limit:** an exported file **leaves the envelope** — it runs **unsandboxed** wherever opened (no CSP, no opaque origin), and substitution-map exports depend on a third-party CDN at view time (bounded by SRI). Acceptable because export is **user-initiated** and the standing **"never bake secrets into an artifact"** rule (D4) bounds what a file can carry; documented, not hidden.
2. **Export *work produced inside a running artifact*** (e.g. a novelist saving a draft typed into the artifact) — a different wall: the sandbox omits downloads, storage, and network (D2), so a running artifact **cannot save out**. This is **not** solved by any hosting/inlining change; it requires the **D4a parent-brokered model-bridge** (artifact `postMessage`s content → PWA → persisted under governance — the `window.claude.complete()` / `window.openai` pattern). Named here as the **first concrete driver** for the D4a bridge; the bridge itself remains a separate flagged capability (a follow-up, not toolkit v1).

### A6 — Reuse by the PWA UX: share the curation, not the hosting (relationship note)

The curated set overlaps the PWA's own rendering needs — and the PWA **already** independently bundles `mermaid ^11`, `prismjs` + `react-syntax-highlighter`, and `react-markdown`/`remark-gfm` (`ChatMessage.tsx`, `MermaidBlock.tsx`), while **lacking KaTeX** (chat LaTeX is unrendered today). The reuse rule, security-permitting:

- **Share the *curation*** (library list + versions + recipes) across both surfaces — pure upside, **zero new security surface**. This converges today's divergence (artifact **highlight.js** vs PWA **prismjs**; artifact **server-SVG mermaid** vs PWA **client mermaid**) and closes the PWA chat-math gap (add KaTeX).
- **Do *not* share the *hosted bytes* for scripts.** The PWA is the **high-trust** origin (full Access session, same-origin authority); the artifact origin deliberately hosts **untrusted LLM bytes**. The PWA loading executable scripts from the artifact origin **inverts the trust gradient** and is only tolerable with **SRI (hash-pin) + the pinned URL + CORS** — and even then the clean default is for the **PWA to bundle its own pinned copies**. (Fonts, being passive/non-executable, are the safe exception to share.)
- `/lib/` stays the **artifact-only sealed shelf**; PWA convergence is a **separate follow-up ticket** (align highlight.js-vs-prismjs, mermaid client-vs-server, add KaTeX), **not** part of the toolkit hosting work, and **not** an expansion of this security ADR's scope.

### A7 — Done-bar (inherits ADR-0089 D5 / ADR-0088 observable-first)

- **E2E under the live CSP:** an artifact that loads hosted `/lib/` libraries (a KaTeX formula + a Chart.js chart at minimum) **renders correctly** in both the drawer and standalone, end-to-end through a real serve + `make verify-envelope` (extended to assert `/lib/` assets serve with the correct executable-MIME + `nosniff` and are reachable under the artifact CSP). "It renders" is not the bar alone; the envelope must still be provably applied.
- **Export verified:** an exported-standalone copy of that same artifact opens and renders **offline** (no network, no Access) with libraries + fonts inlined.
- **The meter is repaired and observable:** the gate-label distinguishes host-allowed `/lib/` loads from host-blocked external `<script src>` reaches (A1) — unmet-capability demand becomes a live, deterministic signal; FRE-498 closed.

### Implementation tickets (filed Needs Approval under the *Artifact Execution Security* project)

Sequenced with dependencies; the build session picks them up after owner approval:

1. **Instrument fix (A1)** — extend the non-load-bearing gate-label to count + classify external `<script src>` reaches (host-allowed vs host-blocked); closes FRE-498. *No dependency; ship first to start the demand signal.*
2. **Host the toolkit (A3)** — `personal_agent_secrets` Worker: KaTeX (+ css + fonts), Chart.js, highlight.js (+ theme), the **OFL font set**, **paged.js** (pending eval-free confirmation) as classic pinned assets; **three.js classic-bundle** sub-task; extend `make verify-envelope` with the `/lib/` reachability + MIME assertion. *Cross-repo.*
3. **Prompt reframe (A4)** — rewrite `_HTML_GENERATION_SYSTEM_PROMPT` to advertise `/lib/` + the native typography recipes; reframes FRE-511's prompt. *Depends on #2 (don't advertise an empty shelf).*
4. **`docs/skills/artifact-design.md` (A4)** — runtime-guidance source-of-truth. *Parallel with #3; #3 distills from it.*
5. **Export-to-standalone (A5)** — Worker endpoint with **two modes**: inline (default, offline-portable) and substitution-map (opt-in, rewrite to public-CDN + SRI), driven by the per-asset **export substitution map** (`{public-CDN URL, SRI hash, CORS-verified, classic|module, fallback=inline}`; three.js + fonts inline-only); documents the envelope-stripping limit. *Depends on #2 (the map is built with the hosted set).*
6. **E2E + export verification (A7)** — KaTeX+Chart.js artifact renders under the live CSP via real serve, and its exported-standalone copy renders offline. *Depends on #2 + #3 + #5.*
7. **(Separate, A6) PWA toolkit convergence** — align PWA rendering libs with the toolkit (highlight.js-vs-prismjs, mermaid client-vs-server, add KaTeX for chat math); shared *curation*, not shared hosting. *Independent; not blocking the artifact toolkit.*

*Out of scope (named, not built here):* the **D4a model-bridge** for saving work produced inside a running artifact (A5 case 2); public/Access-excluded `/lib/`; per-artifact subdomains; sharing *mechanics* (ADR-0089 D6).
