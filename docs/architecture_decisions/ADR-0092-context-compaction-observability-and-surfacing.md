...
**Status:** Accepted — 2026-06-22 (status reconciled by master; impl 4/5 deployed — FRE-568 ✅ projector session aggregate · FRE-570 ✅ A/B/D markers + the 4 `turn_status` fields · FRE-572 ✅ backend monitors · FRE-573 ✅ PWA two-lane render (+ FRE-584 ✅ regression test). **Open:** impl 3/5 **FRE-571** (explicit ES mappings for the new session fields) — **in progress (being built)**. **Parked:** mechanism-C tool-result digest **FRE-569** (Held). → **Implemented** once FRE-571 lands. Originally Proposed 2026-06-15.
