...
- `linear_issue_id` is populated on only **6 of 1,872** reflection docs, mostly since-canceled. The mechanical cause is the ADR-0040 review-bandwidth gate (`promotion.py:279–300`): a hard pause when `count_open_issues > issue_budget_threshold` (200), capped at 5 issues/run (`promotion_initial_cap`). That throttle is *correct by design* — it protects owner review bandwidth — but it is invisible, so conversion looks like a bug rather than a governed rate.
  - **Superseded 2026-09-02 (FRE-1354).** The gate was not merely invisible, it counted the wrong population — *all* non-terminal team issues (259 vs 200), most of which Seshat never created. `issue_budget_threshold`/`count_open_issues` are retired in favour of `seshat_open_ticket_cap` (default 10) over issues carrying the `agent-filed` marker. Separately, D9's read-before-emit turned out to erase the promotion candidate on every reinforcement, so the "min seen_count" criterion could never be evaluated; a reinforcement at or above `promotion_min_seen_count` now survives and carries the canonical row's corroboration.
...
- **Embedder only if needed, and on an always-on path.** System-KG work is **background**, not a hot user turn, and must **not depend on the laptop/Mac-GPU tunnel** (the isolation principle — this guard still holds). If an embedder is needed, **reuse whichever embedder the deployed substrate profile resolves** — today the managed OVH `Qwen3-Embedding-8B` endpoint, permitted by the [ADR-0112 amendment below](#2026-07-06---amended-by-adr-0112-embedder-path--config-default-not-a-mandate) — **or ride FRE-697's ONNX-on-VPS conclusion** — never a separate or laptop-dependent path. *(Superseded detail: this clause originally named the local `embeddings:8503` container as the reuse target. That container is defined but no longer started; the embedding path is managed as of FRE-821. See the 2026-08-05 status update.)*
...
| Embedder scope-creep — the System KG grows a User-KG-style reranker/multi-path stack | Medium | D10 scopes it out explicitly: no reranker; embedder only if the probe passes, on an always-on path (the substrate profile's resolved embedder — managed per ADR-0112 — or FRE-697 ONNX), never laptop-GPU; AC covers "no reranker dependency" |
...
- **D10 separation probe** reuses **FRE-670 / ADR-0103**; **embedder** rides **FRE-697** (ONNX-on-VPS) or the substrate profile's resolved embedder (managed per ADR-0112) — no reranker; **FRE-349** (semantic retrieval into context) is the watched exception; **FRE-710** (coarser reflection cadence) reduces the near-duplicate inflow the dedup must absorb.
...
### 2026-08-05 - Drift correction: the embedder the body named is no longer running
**Changed By:** master (gate on FRE-597 / PR #836)
**Reason:** Factual reconciliation only — no decision is made or reversed here. Four places in this ADR named the local `embeddings:8503` container as the embedder to reuse: the D10 clause, the risk-table mitigation, the Implementation Notes, and **AC-10's check**. That container is defined in compose but no longer started; the embedding path became the managed OVH `Qwen3-Embedding-8B` endpoint under FRE-821 (2026-07-07). AC-10 as originally written was therefore unsatisfiable — it required a run to succeed *using only* a service that does not run.

The design question this raises was **already settled by the 2026-07-06 amendment above**, which is why this is a correction rather than a reopening: ADR-0112 reframed the embedder path from a hardcoded mandate to a configuration default and explicitly permits a managed endpoint, while keeping the **"never laptop-GPU" availability guard** intact. The body text simply was never reconciled with that amendment. All four references now point at the substrate profile's resolved embedder rather than a named container, so they stay correct across future embedder moves.

**Nothing about D10's semantic-dedup measurement gate, the isolated-sysgraph substrate, or the isolation principle changes.** Found while checking, at the owner's prompting, whether anything planned to use the dead public `embeddings` table before dropping it — it did not, but this surfaced alongside. Sibling stale references to the same dead containers are tracked on FRE-1165 (live system prompt) and FRE-1166 (the underlying local-model residue).
