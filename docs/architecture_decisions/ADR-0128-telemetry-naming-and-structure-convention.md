...
- **D5 Tier 2 is Elasticsearch-only, because `default_pipeline` is an Elasticsearch mechanism.** Neo4j and Redis have no equivalent write-time interceptor. Saying "Tier 1 covers them" would be false — Tier 1 is a *telemetry-record* envelope and does not govern a Neo4j property accessor or a Redis stream argument. So each non-Elasticsearch substrate gets its own named mechanism, decided here:
  ...
  - **Neo4j — canonical property name and a migration.** The entity-class property is renamed `class` → **`entity_class`**. `class` is retained as a *deprecated duplicate* for one release so readers migrate without a flag day, then dropped. Existing nodes are migrated by a one-off Cypher `SET n.entity_class = n.class` sweep followed by `REMOVE n.class`; the existing index on the property is rebuilt against the new name. Read sites are `memory/service.py:321` and `:3024`. Enforcement is a repo lint: no Cypher string in `src/` may reference the bare `class` property once the migration lands.
  - **Redis — the prefix belongs to a builder, not to callers.** Stream names today are module constants such as `STREAM_REQUEST_CAPTURED = "stream:request.captured"` (`events/models.py:26`), and `RedisStreamBus.publish` accepts an arbitrary `stream: str` and forwards it to `xadd` (`events/redis_backend.py:96,115`) — so a caller passing a wrongly-prefixed constant is indistinguishable from a correct one. The decision: `publish` stops accepting a free string and takes a **stream-name enum**, with the prefix applied inside the builder. This makes the omitted-prefix failure a type error rather than an empty read.
...
- **A Neo4j property rename with a deprecation window.** `class` → `entity_class` touches read sites at `memory/service.py:321,3024`, requires a node sweep and an index rebuild, and carries a one-release period where both properties exist — during which a reader using the old name still works, which is exactly when it is easiest to forget the second half.
- **A Redis publish-signature change is a breaking API change** inside `personal_agent`: every caller of `RedisStreamBus.publish` must move to the enum, and the free-string escape hatch disappears deliberately.
...
- **AC-3 — The four-field spine is present on every document of every governed family, without discarding records and without washing out real identities.** · **Check:** (a) `exists` on each of `@timestamp`, `event.name`, `trace_id`, `session_id` as a share of `match_all`, **for every governed family — not `agent-logs` alone**; (b) the Elasticsearch ingest-pipeline failure counter and the `slm_server` `es_ship_failed` warning count over the window, both of which must be zero; (c) the share of `trace_id` values that are **real** (neither `system:*` nor absent), compared against the pre-change baseline of 11.4% recorded in the census artifact. · *Fails if* any spine field is below 100% in any family, **or** any record was rejected at ingest, **or** the real-identity share falls below the 11.4% baseline. (c) is the anti-washing check and replaces a fixed sentinel ceiling, which would have been unachievable: sentinel-filling today's measured absence necessarily yields ~88% sentinels on day one, so a ceiling would have failed a correct implementation. What must not happen is a *real* identity being replaced by a sentinel. (b) replaces a percentage volume floor, which the census's own daily figures (20,876–72,665/day) show cannot discriminate a drop from ordinary variation — measuring rejections directly measures the risk instead of proxying it.

- **AC-4 — Violation tagging demonstrably fires, and the production rate is bounded and falling.** · **Check:** (i) *positive control* — write a record whose emitting subsystem is unidentifiable and confirm it is indexed with a fully-populated spine and `telemetry.convention_violation` naming the guessed fields; (ii) production violation count divided by total documents, per family, per week, published by the joinability monitor, across three consecutive weeks. · *Fails if* the positive control is not tagged (a tagger that never fires would otherwise produce a flat zero rate and pass), **or** the production rate exceeds 1% in any family, **or** the rate rises across three consecutive weeks. The positive control is what stops "zero violations" meaning "no detector".
...
- **AC-9 — Every live normalisation rule has a still-nonconforming producer, and every deleted rule has a conforming one.** · **Check:** for each rule in the D5 Tier-2 pipeline `_meta`, read `owning_producer` and the retired spelling, then **sample that producer's own index directly** for documents still carrying the retired spelling over a 7-day window; `retired_by` ticket state is corroborating evidence only. · *Fails if* a rule is live while its producer's sampled output shows zero occurrences of the retired spelling (the layer outliving its purpose), **or** a rule has been deleted while its producer's output still shows the retired spelling. Sampling per rule rather than deferring to AC-2 is deliberate: AC-2 covers `slm-requests` alone, so a rule for any other producer could otherwise be deleted prematurely undetected.

- **AC-10 — The two prior open items are closed behaviourally.** · **Check:** (a) `event.name` present on 100% of documents across governed families, **and** zero occurrences of the dual-key fallback pattern in `src/` — both, since deleting the fallback proves nothing about what is emitted; (b) the Elasticsearch work-taxonomy field's distinct values over a 7-day window are a **subset of the enumerated Postgres `purpose` vocabulary**, and per-value counts reconcile against the Postgres cost ledger for the same window within 5%. · *Fails if* `event.name` coverage is short, the fallback survives, or the Elasticsearch distribution contains a value absent from the Postgres vocabulary or diverges from the ledger beyond tolerance. Source-to-sink parity replaces a dominance ceiling, which would have measured traffic mix rather than correctness — a genuinely single-purpose week is not a defect, and a broken producer could fabricate minority values to duck a ceiling.

- **AC-11 — The convention reached the non-Elasticsearch substrates, mechanism and history both.** · **Check:** (a) *Neo4j* — every entity node returns a value for `entity_class` (`MATCH (e:Entity) WHERE e.entity_class IS NULL RETURN count(e)` = 0, over **all** nodes, not only newly written ones), the deprecated `class` property is absent after its one-release window, and no Cypher string in `src/` references the bare `class` property; (b) *Redis* — `RedisStreamBus.publish` no longer accepts a free-form `str` stream name (`ast-grep` for a publish call passing a string literal or a bare variable returns nothing), so an omitted prefix is a type error rather than an empty read. · *Fails if* any entity node lacks `entity_class`, or `publish` still accepts an arbitrary string. Checking the *signature* rather than call-site literals is what makes this real: a literal-only check would pass a call using a wrongly-prefixed constant, which is exactly the failure observed.
