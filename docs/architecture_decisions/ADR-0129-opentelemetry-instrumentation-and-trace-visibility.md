...
- Ties trace visibility to Kibana, which is not the owner's chosen trace UI — building the target on a surface not chosen for it. *(Originally worded "which is being retired… a surface with a scheduled end"; the 2026-08-07 deferral removed the scheduled end, and the Status Update of that date records the correction. The ruling retains Kibana and permits limited development on it, so the objection is about which surface was **chosen** as the trace UI, not about the owner declining to use Kibana at all.)*
...
- **AC-10 — The owner can reach Grafana at its own gated host, an unauthenticated request cannot, and `monitoring` still serves Kibana.** · **Check:** (a) a request to the `observe` host carrying a valid Cloudflare Access session returns the Grafana UI, and a **named** provisioned dashboard renders with at least one panel whose datasource query returns without error; (b) `GET /api/user` over that session reports an **anonymous identity whose role is exactly `Viewer`** — `Editor` and `Admin` would equally present no login screen, so "no login appeared" does not establish the configured role; (c) the same request **without** a valid Access session never reaches the container, established by a **positive control**: two requests to a unique nonce-bearing path, one session-carrying and one not, where Grafana's router log (D6 requires it enabled) records the first and not the second; (d) the **live** `monitoring` host is requested and behaviourally identified as Kibana. · *Fails if* the unauthenticated request reaches the container, the anonymous role is anything but `Viewer`, the named dashboard's panel query errors, **or** the positive control's *authenticated* request is also unlogged — which makes the check **inconclusive, never passing**. **(c) and (d) are where this criterion earns its keep.** Absence of a log line proves nothing on its own: Grafana does not log successful requests unless router logging is on, so silence is equally consistent with a working gate and with no logging at all — hence the positive control. And (d) reads the **live route**, not `docker-compose.cloud.yml`'s comment: the ingress mapping lives in the Cloudflare dashboard, so the comment can remain accurate-looking long after the route was repointed. This criterion requires an owner action — the tunnel ingress rule and the Access policy live outside this repository and outside CI — which is permitted of an ADR's *own* criteria (ADR-0130 D1) and is precisely why no implementation ticket can prove it.
