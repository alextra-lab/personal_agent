...
**The orchestrator durably records each turn's tool count in Postgres `session_events`.** This is one small write, decided here rather than left to implementation, because without it AC-5's tool-span reconciliation would enumerate its expected count from `tool_call_started` log records emitted by the very code path being rewritten — so if that path stopped emitting, the criterion would pass **vacuously on an empty population**. The expected count must live in a store that cannot disappear together with the thing under test.
...
- `src/personal_agent/orchestrator/executor.py` — additionally persist each turn's tool count to Postgres `session_events` (D3), the independent expected-count source AC-5 reconciles against.
- **`scripts/audit/adr0129_trace_verifier.py`** — new, committed with the implementation. The acceptance criteria are population-level and cross-store (Postgres → Tempo → Elasticsearch); this is the artifact that runs them, so each criterion is executable rather than aspirational. It follows the precedent of `scripts/audit/fre1038_naming_census.py`. It also **writes and reads a baseline file** holding the two measured quantities the criteria judge against — the exporter's steady-state delivery loss over the first 48 hours, and the app-host↔Collector clock skew — so no tolerance in this ADR is a number invented at authoring time.
...
**Three rules govern every criterion below, and they exist because the obvious formulations are all satisfiable by a broken system.**

1. **Population is enumerated from Postgres, never from the telemetry under test.** Turns come from `session_events` (`session_id` + `seq`), model calls from `api_costs`. Enumerating from Elasticsearch log records emitted by the code being rewritten means that if that code stops emitting, the population empties and every criterion passes vacuously. Postgres cannot disappear together with the thing under test.
2. **Structural correctness carries zero tolerance; delivery is measured separately.** A mis-parented or malformed span is a defect, not noise, so no percentage allowance applies to *shape*. Span **loss** is a different quantity: the exporter's drop rate is measured over the first 48 hours after cutover and **recorded in the verifier's baseline file**; steady-state loss is then judged against that measurement. No delivery threshold is invented in this document, because a number picked now would be either unachievable or meaningless.
3. **Every criterion is evaluated per stratum, and empty strata are named rather than skipped.** Strata are the natural partitions — entrypoint (served turn, scheduler, consolidation, monitor, startup), model provider/client, and `slm_server` emit path. A global percentage lets one wholly uninstrumented low-volume path hide inside the allowance, which is exactly the failure this ADR exists to eliminate. A stratum with no traffic in the window **fails the run unless it is explicitly listed as inactive in the verifier output**, so an untested path is visible instead of silently absent.

- **AC-1 — Every tool-using turn renders as a correctly shaped waterfall.** · **Check:** enumerate tool-using turns over the window from **Postgres `session_events`** (turns whose recorded tool count is ≥1); for each, fetch the trace from Tempo and assert the shape **root → step → {model-call, tool-call}**, with model-call and tool-call spans as *siblings* beneath the step span and every span having its expected parent. · *Fails if* **any** enumerated turn whose trace was delivered has a missing parent link, a single-span trace, or a tool span parented to a model-call span. Turns with **no trace at all** are counted separately as delivery loss and judged against the recorded baseline (rule 2) — availability and structural correctness are different failures and are not allowed to mask each other. The shape assertion is the discriminator: unparented spans still render a trace view, and that is today's defect drawn in colour.

- **AC-2 — Correlation resolves in both directions, per stratum.** · **Check:** for each entrypoint stratum, (a) every distinct `trace_id` appearing on that stratum's log records in the window resolves to a trace in Tempo; (b) for every stratum, at least one trace is followed through Grafana's trace-to-logs link and returns non-empty Elasticsearch records whose `trace_id` equals the span's. · *Fails if* any stratum has unresolvable trace ids beyond the recorded delivery baseline, or any stratum's trace-to-logs direction returns empty. Per-stratum evaluation is what stops a wholly uncorrelated background entrypoint hiding inside a global percentage; (a) is what catches a `trace_id` that is present but fabricated, since a stamped literal satisfies an `exists` query and resolves to nothing.

- **AC-3 — Identity is a property of the runtime, and each trace belongs to exactly one turn.** · **Check:** over the window, (a) **per entrypoint stratum**, the share of post-bootstrap log records carrying a `trace_id` that resolves in Tempo, against the 11.36% baseline; (b) for every turn enumerated from `session_events`, all log records correlated to that turn share **one** `trace_id`, and that `trace_id` appears on **no other turn** — turn membership established from the Postgres record, never from the trace id under test; (c) records emitted before SDK bootstrap are enumerated **by logger name** and reported as an explicit excluded list, not absorbed into a tolerance. · *Fails if* any stratum's resolvable share is below the delivery baseline, **or** any turn's records span multiple trace ids, **or** any `trace_id` is reused across turns, **or** (c) is unenumerated. (b) is the criterion that carries the weight: a producer stamping one valid, resolvable id onto every record passes a bare presence-and-resolvability test, and only independent turn membership rejects it.

- **AC-4 — Elapsed time has one representation, and spans are measurements rather than placeholders.** · **Check:** (a) zero records written after cutover carry `duration_ms` or `latency_ms`; (b) **per model provider/client**, model-call span count reconciles against that provider's row count in Postgres `api_costs` for the window, within the recorded delivery baseline; (c) **per provider**, sampled model-call spans reconcile start and end against that call's `api_costs` timestamp and recorded duration, within a tolerance derived from the measured clock skew between the app host and the Collector, recorded in the baseline file. · *Fails if* a legacy field appears post-cutover, any provider's counts diverge beyond baseline, or any sampled span's timing diverges beyond the recorded tolerance. (a) alone is satisfied by a producer that stopped emitting anything; (b) alone is satisfied by one dummy span per ledger row with fabricated timing; per-provider stratification stops a wholly uninstrumented client hiding behind a busy one.

- **AC-5 — The tool span tree is complete and correctly parented, for every tool-using turn.** · **Check:** for each turn enumerated from `session_events`, N is that turn's **Postgres-recorded tool count** (D3); assert the trace contains exactly N tool spans, each a child of the step span, and no child span's duration exceeds its parent's. · *Fails if* any turn's tool-span count differs from N, any tool span is not a child of the step span, or a child outlives its parent. Taking N from Postgres rather than from `tool_call_started` log records is what makes this non-vacuous: the log path is the code under test, and a criterion whose population empties when the tested code stops emitting proves nothing.

- **AC-6 — `slm_server`'s work appears inside the calling turn's trace, on every active emit path.** · **Check:** after the `slm_server` change, over the window, **per emit path** (chat, responses, rerank, streaming): (a) spans exist whose `trace_id` equals the calling turn's, and the per-path span count reconciles against that path's row count in Postgres `api_costs` — an independent ledger that does **not** traverse the new exporter — within the recorded delivery baseline; (b) no new `slm-requests-YYYY.MM.DD` index is created. · *Fails if* any path with traffic shows no shared-trace propagation, any active path's count diverges beyond baseline, or a new dated index appears. A path with **zero** traffic in the window does not pass by default: it is listed as inactive in the verifier output and requires either an extended window or a declared production canary before the criterion is accepted. (a) is unfakeable from inside this repository — only genuine cross-process propagation produces a shared trace.
...
- **AC-8 — "Did responses get faster?" is answerable from one query over a fortnight.** · **Check:** a Grafana panel plotting per-day p50 and p95 turn duration from spans returns a non-empty series for **every day on which `session_events` records served turns** across a ≥14-day post-cutover window, computed **without unioning two fields**; and each active day's turn-span count reconciles against that day's `session_events` turn count within the recorded delivery baseline. · *Fails if* an active day is empty, answering requires more than one duration field, or a day's span count diverges beyond baseline. Continuity is defined over *active* days because a genuinely idle day is not a defect and a criterion that failed one would be unachievable. The count reconciliation is load-bearing: one surviving span per day yields a continuous series that is a lie. This criterion also depends on D6's committed `max_duration` configuration — a 14-day metrics query fails against Tempo's 24-hour default.
...
- `src/personal_agent/service/models.py:408` (`session_events`, `session_id` + `seq`) and `docker/postgres/init.sql` (`api_costs`) — the Postgres sources every criterion enumerates from, chosen because they do not traverse the telemetry path under test
