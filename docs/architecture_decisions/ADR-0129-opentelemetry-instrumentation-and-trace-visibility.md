...
- **AC-7 — Collector redaction demonstrably fires, and no producer bypasses it.** *(Clause (ii) reworded and clause (iii) added 2026-08-09 for the loopback-hop topology D5 now permits; see the Status Update of that date.)* · **Check:** (i) *positive control* — emit a span carrying an attribute matching a declared redaction rule and confirm the attribute is **absent** in Tempo; (ii) every **span-originating** producer publishes a **machine-readable effective-configuration artifact** that the verifier reads — including `slm_server`, whose ticket must expose one, since it runs on a separate host the verifier cannot otherwise inspect — the artifact enumerates that process's **complete configured span-exporter set** rather than a single representative endpoint, and **every** destination in that set is one of exactly two admissible classes: the VPS Collector's ingress, or a **same-host loopback** Collector permitted by D5; (iii) where an artifact names a loopback hop, that producer's spans **reach Tempo in reconciled volume** — its span count over the window agrees with that producer's own independently enumerated `api_costs` population within the bounded delivery baseline (rules 1, 2 and 4 apply in full, so an implausibly small population fails as **inconclusive**, never as a pass). · *Fails if* the planted attribute survives to storage, any artifact names a destination outside (ii)'s two admissible classes, any artifact reports a single endpoint where the process has an exporter *set*, **any producer publishes no artifact** (an uninspectable producer is a failure, not an exemption), or a producer whose artifact names a loopback hop diverges beyond the delivery baseline — **or has no independent ledger population against which to reconcile at all, in which case (iii) is inconclusive rather than satisfied.** The positive control is required because a redactor that never fires yields the same clean result as one that works.

  **Why (ii) says *every* destination, and why (iii) reconciles rather than merely observes.** The original wording — *"no artifact names an OTLP endpoint other than the Collector"* — is singular, so an artifact reporting one compliant endpoint alongside an unreported second exporter arguably satisfied it; the closure form, plus the explicit demand for the complete set, does not. (iii) stops the loopback allowance passing vacuously: a criterion accepting the producer's own `localhost` value with nothing behind it could not fail, and a bare *presence* check is the failure AC-8 already names in this document — one surviving span yields a result that is a lie. Reconciliation against `api_costs`, the ledger that does not traverse the path under test (rule 1), is what makes the clause discriminating.

  **Three limits are named rather than implied, because each is a way this criterion can pass while something is wrong.** *First*, (ii) is self-report: it is decidable only over what the process chooses to publish, and an exporter withheld from the artifact defeats it. That was equally true of the original wording and is not introduced here — but the closure form makes the artifact's **completeness** the load-bearing property, so it is stated. *Second*, (iii) proves **reachability, not exclusivity**: a hop splitting its output, most to the terminus and some elsewhere, reconciles within tolerance and passes. *Third*, combining the two, **a second destination configured on the intermediate hop is invisible to this criterion** — the vanilla Collector publishes no config to read (D5), so no clause here can see it. This is a capability limit, accepted deliberately as the price of the hop, and it is why the allowance is bounded to a Collector on the producer's own host. For `slm_server` specifically, (iii) restates AC-6(a)'s reconciliation at AC-7's scope, which keeps this criterion self-contained; the two are not independent evidence and must not be counted as such.
