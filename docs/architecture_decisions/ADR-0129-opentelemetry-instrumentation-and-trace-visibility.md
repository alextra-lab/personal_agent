...
it**; that state is unmonitored by design and by owner ruling. **A replacement clause was considered
and rejected**: "`monitoring` does not serve Kibana" is true by construction once no Kibana exists,
which makes it unfalsifiable and therefore barred by this ADR's own criteria bar. **Seam ticket
