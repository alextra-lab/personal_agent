...
`es_logger.log_event` has exactly **one** production caller in `src/` — `telemetry/es_handler.py:205`. Every `agent-logs` document therefore passes through one function, which is what makes seam-level enforcement viable at all rather than requiring 551 emit-site edits.

**The path has two properties that together fix where a validator can go, and neither is obvious from the outside.** First, `es_handler.emit` handles two branches: records whose `record.msg` is a dict, which came through structlog, and — at `es_handler.py:86` — a fallback reading `record.__dict__` for plain `logging` records that never entered structlog at all. Anything registered in `structlog.configure` sees only the first. Second, the stored document does not exist anywhere in `es_handler`: `emit` schedules `_log_async` (`:172`), which begins at `:181` and calls `log_event` at `:205`, and only inside `log_event` is `doc` assembled — merging `@timestamp`, `event_type`, `trace_id` and `span_id` over the caller's payload (`es_logger.py:165-171`).

So a validator upstream of `log_event` would either miss a branch or check an object that is not what gets stored, including two governed names it would never see. This is recorded in Context because it decides D2's placement, and getting it wrong would have produced criteria that fail a correct implementation.
...
### D2 — Enforcement lives where the stored document is assembled, never at 551 call sites and not in the structlog chain

The envelope is a **validating step inside `es_logger.log_event`**, applied to the `doc` dictionary immediately after it is assembled at `src/personal_agent/telemetry/es_logger.py:165-171` and before it is handed to `client.index`.

**Two candidate seams were rejected, each for a different reason, and the placement follows from what each one can actually see.**

- **The structlog processor chain** (`telemetry/logger.py:232`, alongside FRE-1064's span-context processor) misses records. `es_handler.emit` accepts records whose `record.msg` is a dict — the structlog path — **and, at `es_handler.py:86`, falls back to reading `record.__dict__` for plain `logging` records** that never traverse structlog at all. A validator registered there would silently skip that branch, and AC-2 would then fail a correct implementation.
- **`es_handler`'s own path** sees the right records but the wrong object. `emit` schedules `_log_async` (`es_handler.py:172`), which begins at `:181` and calls `log_event` at `:205` — but the stored document does not exist yet at any of those points. It is assembled inside `log_event`, which merges `@timestamp`, `event_type`, `trace_id` and `span_id` over the caller's `data` (`es_logger.py:165-171`). A hook before `log_event` would validate the payload the caller passed, **not the document Elasticsearch stores**, and the four merged keys — which include two governed names — would go unchecked.

`log_event` is therefore the only point that sees every record *and* the final document. It has exactly **one** production caller, so placing the validator inside it makes coverage of `agent-logs` total rather than approximate. `log_batch` on the same class has no production caller today; if it acquires one it carries the same validation, and that is a standing constraint rather than a note.
...
**Why Rejected:** It is the status quo, and the status quo's cost is documented across five ADRs. The distinguishing property of this decision is that its mechanism is a single validating step inside the one function that assembles every stored `agent-logs` document — days of work, not a programme.
...
- **The other governed families are not reached.** Captures, reflections, insights, joinability and slm-health write through their own paths, never reach `es_logger.log_event`, and keep the naming freedom ADR-0129 left them. D1 states this rather than claiming corpus-wide governance, but it is a genuine hole: the divergence class remains fully available in those families.
...
- `src/personal_agent/telemetry/es_logger.py:165-171` — apply the validator to `doc` immediately after assembly and before `client.index`; this is the only point that sees both `emit` branches *and* the four keys `log_event` merges in.
...
**Migration steps:** declare the vocabulary (seeded from the 59 cross-family names and the five recorded divergences) → add the validator to `log_event` behind its development-time behaviour → publish the production counter → retire `CANONICAL_MODEL_CALL_*_FIELDS` into it as FRE-1067 lands.

**Dependencies:** FRE-1064 (ADR-0129 B1 — the chain head this sequences behind, and the falsification gate D7 binds this work to; `Approved`, `stream:build1`) · FRE-1067 (ADR-0129 B3 — deletes the canonical field sets, and carries the AC-12 wording D6 rules on; `Approved`) · FRE-1037 (role-enum widening, supplying the `purpose` vocabulary) · FRE-1045 and FRE-1048 are **closed** by D5 and D6 respectively.
...
- **AC-2 — The validator ran over the production corpus, and applied the whole declared vocabulary to ordinary traffic.** · **Check:** over the window, (a) *coverage* — the validated-record count published per D4 equals the documents indexed into `agent-logs` **plus** the handler's own recorded delivery failures (`es_handler._record_failure`) for the same window; (b) *differential replay* — draw a sample of at least 10,000 stored `agent-logs` documents from the window, re-run the **committed vocabulary and rules offline** over them, and compare the violation set that replay predicts against the violations the production counter actually recorded for those documents; (c) *planted control* — a record carrying a declared retired spelling, emitted through the live service, increments the violation counter. · *Fails if* (a) does not reconcile exactly, **or** (b) diverges in either direction on any document, **or** (c) does not increment. **This is the criterion the rest depend on, and it is stated first among the production checks for that reason.**

  Each leg answers a different cheat, and the third is the one earlier drafts missed. (a) alone is satisfied by a no-op step that increments a denominator and applies no rule — it would run on every record and report perfect coverage. (c) alone is satisfied by an implementation that runs the full rule set in CI while production recognises only the one spelling that was planted; **(b) is what forbids that**, because a partial production validator predicts a different violation set than the committed rules do, on real traffic it did not choose. Divergence "in either direction" is deliberate: replay finding violations production missed is under-enforcement, and production reporting violations replay does not is a rule that is not the committed one.

  Reconciling against delivery failures in (a) rather than demanding bare equality is what stops a correct validator failing on unrelated loss: `_log_async` returns early when disconnected or when the circuit is open (`es_handler.py:196-199`), and `client.index` failures are caught and counted (`:205-211`) — none of which is the validator's doing.

- **AC-3 — No declared retired spelling survives in stored production documents.** · **Check:** for each retired spelling in the vocabulary, an `exists` query over `agent-logs` documents for the post-cutover window — over documents, not mapping declarations. · *Fails if* any returns non-zero, other than the single record planted by AC-2(c), which is named in the verdict. Mapping-absence is insufficient: a retired field persists in `_source` under a dynamic type with no declaration at all. This carries force **only in combination with AC-2**, which is what distinguishes "Rule 1 held" from "no producer happened to emit one this week."

- **AC-4 — The validator neither dropped nor altered a record.** · **Check:** (a) the shortfall between AC-2's validated count and the `agent-logs` indexed count is **fully attributed to the handler's recorded delivery failures**, itemised by cause (disconnected, circuit open, index error); (b) the record planted by AC-2(c) is present in Elasticsearch with the offending key intact and its value unchanged; (c) across AC-2(b)'s replay sample, every document's stored `_source` still contains every key the validator saw. · *Fails if* any part of the shortfall is **unattributed** — a residue not explained by a delivery failure is a validator-caused drop and fails regardless of size — **or** the planted record is absent, altered or stripped, **or** any sampled document lost a key. **The bar is zero validator-caused loss, not zero loss:** an earlier draft failed only if a shortfall was "non-empty *and* unenumerated," which permitted known dropped records under a criterion whose title forbids them, and the draft before that allowed 1% — about 5,000 records a week on this corpus. (c) closes the sanitising path at population scale rather than through the single planted record.

- **AC-5 — The violation rate is published, bounded, and shrinking while it matters.** · **Check:** violation count divided by **AC-2's validated count** (never by an assumed total), published by the joinability monitor, over three consecutive weeks. · *Fails if* the rate is unpublished for any week, **or** exceeds 1%, **or** is at or above 0.1% and has not declined across the three weeks. A genuinely-zero rate is success and is not required to decline — but only because AC-2(b) has proven the production rules match the committed ones on ordinary traffic and AC-2(c) has proven the counter increments when a real violation passes through, so a zero means "none occurred" rather than "none was looked for."
...
- Linear FRE-1064 — ADR-0129 B1, the chain head this sequences behind and whose falsification gate also governs this work; `Approved`, `stream:build1`
