...
3. Stage 7 discarded the context to fit the token budget. The drop itself is
   `request_gateway/budget.py:269`, which sets `memory_context = None`. `pipeline.py:148-156`
   only maps the recorded action onto telemetry phases afterwards.
...
#### Reason one: a standing layer populates the context on every turn
...
So on any authenticated turn where that fetch succeeds, `memory_context` is non-empty regardless
of what recall found. A status derived from "is `memory_context` non-empty" would read `POPULATED`
on essentially every turn. This is the ADR-0147 §D3 defect in a new place, and it is decisive for
the design: the status must describe the **recall layer**, not the container.
...
embedding term of zero. A top-ranked non-match does not score orthogonal. It scores near 0.706.
...
- **Broad recall**, for `MEMORY_RECALL` intent (`context.py:409`). Its relevance floor is
  `recall_similarity_floor`, which defaults to **0.0** — *"no floor (legacy-equivalent)"*
  (`config/settings.py:644-653`). ADR-0100 requires it to be embedder-calibrated. It is not
  calibrated today.
...
| `WITHHELD` | Recall admitted items. The budget drop or the renderer removed all of them. | *"I hold records on this and could not fit them into this turn."* |
...
2. `WITHHELD` outranks `POPULATED` only when **every** admitted item was removed. If any item
   survived to the render, the state is `POPULATED`.
3. `NOTHING_RELEVANT` is reachable only when every arm ran to completion and admitted nothing. It
   is the one state that requires positive evidence of a complete run.
...
`WITHHELD` stays separate from `UNAVAILABLE` because a budget or render drop is the one
non-populated case where the system **knows** items existed. That is strictly more information than
a failure supplies, and it is actionable for the reader, who can narrow the question and ask again.
...
### D4 — Every admitting path carries a relevance predicate that recency cannot satisfy

The obligation is stated once, at the level all three paths share:

> No path admits a candidate on recency, name resolution or any non-relevance signal alone. Each
> admitting path holds a relevance predicate able to reject a candidate whose relevance evidence is
> at or below a calibrated bound, whatever its age.

What that means per path, because the three do not share a scoring model:

- **Proactive** — a hard gate ahead of `_combine_scores` (`memory/proactive.py:113-127`). A
  candidate with no entity overlap, no topic hit, and an embedding term at or below the calibrated
  bound is not admitted, whatever its recency. The gate precedes the combination rather than
  adjusting weights inside it, so recency cannot compensate for it.
- **Broad recall** — `recall_similarity_floor` is calibrated and non-zero. ADR-0100 already
  requires this and `config/settings.py:645` still ships `0.0`. No new mechanism is needed. The
  existing one is switched on with a calibrated value.
- **Entity match** — this path computes no similarity today. It gains one relevance predicate over
  the `query_text` it already carries, or its admissions are marked as name-resolved rather than
  relevance-established. **This ADR does not decide which.** The implementation ticket decides it
  against the measured population of that path, and the choice is recorded there.

**The bound comes from a calibration, not from this document.** The FRE-694 medians were measured
in June on the 0.6B@1024 arm. Production now runs the OVH-managed 8B model, whose Youden J was 0.53
against 0.6B's 0.42 — better separation, and still overlapping clouds on every arm tested. The
...
- It runs against the deployed embedder arm, over a labelled probe set, and reports the positive
  and negative distributions in Neo4j score space, as FRE-694's harness does.
- The bound is chosen so the **median top-ranked non-match is rejected**.
- It reports the share of labelled positives the chosen bound admits. AC-5 binds that share.
...
set of barely-admitted items. The gate stops that set from being assembled, so the band addresses a
population the design removes.
...
- The entity-match path's predicate is left to its implementation ticket, so this ADR does not close
  every path by itself.
...
| The gate lands on one path and the defect moves to another | High | D4 states the obligation at the shared level and names all three paths. AC-4 covers proactive and broad recall; the entity-match predicate is its ticket's own criterion. |
| The calibration has no owner and never happens | Medium | D4 makes a missing calibration a `config_guard` finding that holds the previous bound rather than defaulting to zero. |
...
- `src/personal_agent/memory/proactive.py` — the D4 gate ahead of `_combine_scores`, plus its own
  drop reason.
- `src/personal_agent/config/settings.py` — `recall_similarity_floor` takes its calibrated value.
...
- **AC-2** — A failed recall and an empty recall are distinguishable in the context the model
  receives, and a partial failure ranks as failure. · **Check:** take one probe whose subject is
  verified **present**. Run it three times: unmodified; with `generate_embedding`
  (`memory/service.py:5059`) raising for every arm; and with it raising for one arm while another
  returns items. · *Fails if* the unmodified run is not `POPULATED`, or either failure run is not
  exactly `UNAVAILABLE`, or the total-failure run renders identically to a `NOTHING_RELEVANT` turn.
...
- **AC-4** — The relevance predicate binds on the measured non-match, not on orthogonal, on both
  paths that have one. · **Check:** (a) a proactive unit test at the calibrated top-ranked-non-match
  median in Neo4j score space, with zero entity overlap, zero topic hits and a same-instant
  timestamp, against the deployed weights; (b) a broad-recall test at the same score with
  `recall_similarity_floor` at its configured value. · *Fails if* either candidate is admitted. Each
  test carries a companion assertion that the same fixture is admitted with the predicate disabled —
  a test that passes both before and after the change proves nothing.

- **AC-5** — The bounds are calibrated, and they do not suppress genuine relevance. · **Check:** apply
  each configured bound to the calibration's labelled positive set. · *Fails if* the bound does not
  reject the median top-ranked non-match, or if it admits fewer than **90%** of labelled positives at
  the p50 of the positive distribution. Both halves must hold; the first alone permits a bound that
  rejects everything, and the second alone permits a bound of zero.
...
- **AC-7** — In each of the three non-populated states, a memory-derived assertion has no admissible
  referent. · **Check:** construct one turn in each of `NOTHING_RELEVANT`, `WITHHELD` and
  `UNAVAILABLE` where the model asserts a fact about the user's history; query the grounding record
  for that span in each. · *Fails if* the span resolves to a memory source in any of the three, which
  would mean the registry was populated in a state that claims it is not.
...
- **AC-8** — The standing behavioural-stance layer does not defeat the absence state. · **Check:** an
  ...
  none; read the status and the rendered sections. · *Fails if* the status is `POPULATED`, or if the
  behavioural section's presence suppresses the `NOTHING_RELEVANT` line. This criterion fails against
  today's code, which is why it is here.

- **AC-9** — The type change does not silently break the ADR-0147 digest. · **Check:** build a
  planner digest on a turn in each of the four states. · *Fails if* the digest raises, or if a
  non-populated state produces a digest with items in it.
...
- ADR-0100 — `recall_similarity_floor`, which D4 requires calibrated rather than zero
...
that absence is not reachable today. Codex round 1 then corrected the reachability argument's
statistics, found the standing-stance layer as an independent and stronger cause, and found two of
the three admitting paths ungoverned by the first draft's gate.
