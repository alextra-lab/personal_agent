...
| [ADR-0143](ADR-0143-the-request-task-is-the-unit-that-must-end.md) | The Request Task Is the Unit That Must End — Bound the Service Task, Not the Turn (the chat turn task is created through a service registry, carries an outermost wall-clock bound above the orchestrator's cap, and closes the client stream at the bound even when its own persistence is down; the orchestrator cap's description is corrected to its real bind points and gains a per-step check; measured the FRE-1403 "hang" and found a completed turn plus two adjacent defects filed separately) | Proposed |
