...
| [ADR-0151](ADR-0151-a-turn-that-cannot-be-grounded-says-so.md) | A Turn That Cannot Be Grounded Says So — Enforcement Narrowed to the Turns a Retry Can Repair (FRE-1328, from the owner's 2026-09-13 decision "states it could not ground": a 91-turn measurement finds no turn fully passed, arbitrary-code turns are 23 of 91, the no-tool shape is 30 turns of which 16 are eval and the rest general knowledge and conversation, and the largest shape is turns that retrieved typed sources and did not cite them; so turns are classified after generation from the registry counts into shape A (tools refused), B (a tool source admitted) and C (no tool result), `enforce` retries shape B once with a cite-only directive and no retrieval grant, shapes A and C never retry, `TERMINAL_NO_SOURCE` is withdrawn so every turn ships with a deterministic per-shape declaration in both modes, statements the verifier could not settle neither trigger a retry nor count as unsourced in the FRE-1325 note, and ADR-0138 D5's pre-generation forcing is withdrawn; it does not detect fabrication, so FRE-1361 stays open) | Accepted |
| [ADR-0152](ADR-0152-the-planner-decides-whether-to-expand.md) | The Planner Decides Whether to Expand — Asked, It May Decline (FRE-1502, from FRE-1498's finding that the planner, given the option, declined every command and made ten of the twelve research expansions itself, the other two coming from the fallback planner after a planner failure, and a 2026-09-14 planner-probe matrix across llama.cpp and MTPLX, Flash-Next, the 27B, the 35B-A3B and Arch-Router, quants and effort levels, in which the qualifying planners agree with the expected decision on 91–98% of draws, higher effort never helped, a router in front scored 65%, and the planner call costs 13–23 s per turn on the best configurations; so D1 routes `CONVERSATIONAL`, `TOOL_USE`, `ANALYSIS` and `PLANNING` to the planner as `HYBRID` with a decline rule, a decline or a planner failure returns to the tool loop and never reaches the fallback planner, D2 bounds expansion at min(3, the load budget) with `DECOMPOSE` dispatched as `HYBRID` and round budgets 3/6/10, D3 renders the enforced budgets without claiming an effect, D4 records the planner decision per turn and keeps declines out of the FRE-514 label-lie predicate, D5 keeps the planner on the primary's model and mode and allows a planner-specific temperature only on test (0.6 scored 89% against 96% at 1.0, so none is adopted), and D6 qualifies a planner configuration on the committed probe; amends ADR-0036 D1/D5, supersedes ADR-0142 D1's routing half, and records the OVH primary as unqualified until probed at production effort) | Proposed |
