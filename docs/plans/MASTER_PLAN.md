...
> **Last updated**: 2026-07-05 (V2 cutover complete + ADR-0110 T1/T2/T3 shipped). **ADR-0109 V2 taxonomy cutover SHIPPED end-to-end this session:** the 10-type extractor (FRE-771) + recall-consumer remap (FRE-794) are live in the gateway (rebuilt 09:54 from main), and the FRE-772 KG migration ran against prod Neo4j — **0 Technology/Topic/Concept remnants, joinability probe green** — batched + cost-gated via FRE-800/FRE-801 (~$0.43, ~38× fewer calls). **Correction:** the prior `0f46a84`-deployed note was inaccurate (baked from a stale tree still serving V1); the real V2 deploy is this session's rebuild. Also shipped this session: **FRE-796** web-tools fix (Perplexity→`sonar-reasoning-pro` + searxng chefkoch category-leak) live · **FRE-798** ruff-hook `--unfixable F401` · **FRE-677/795** test-hygiene · **FRE-790** boundary probe (κ 0.858) · **FRE-797 = no-change** (boundary clears; residual is a cross-provider artifact a definition can't fix). ADR-0099 config management **Implemented** (FRE-648–652 Done). **ADR-0110 dispatch automation — T1/T2/T3 shipped this session:** FRE-785 resolver, FRE-786 (RC-mechanics spike proven live owner-present + launcher `scripts/dispatch/launcher.py`), FRE-787 orchestrator loop (`scripts/dispatch/orchestrator.py`) all Done. **Live blocker found at the FRE-787 gate:** the shared `fetch_board` GraphQL query returns HTTP 400 against the live Linear API (T1 merged on mocked tests, never live-exercised) → **FRE-804** (Urgent) fixes it before FRE-788 (T4 ops) + the master-owned live assembled-seam demo. FRE-699 recall/rerank research shipped (PR #383, Done). **Dispatch now:** build1→**FRE-804** (fetch_board GraphQL 400 — live-verify, Sonnet) · build2→**FRE-620** (KGQ quality-monitor `:Conversation`→`:Turn` fix + anomaly recalibration + promotion floor, Sonnet) · adr **idle**. **Open owner items:** FRE-803 (init.sql↔model parity red, Needs Approval) · **MedicalCondition** entity-type seed (health-domain, parked — Seshat-as-personal-collaborator) · config-UI capstone. Process-v2 baseline: Linear-native dispatch `stream:*`+priority+blocked-by; PR-merge→Awaiting Deploy. Prior narrative → [`completed/2026-07-04-master-plan-archive.md`](completed/2026-07-04-master-plan-archive.md).
