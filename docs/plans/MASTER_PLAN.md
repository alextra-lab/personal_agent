...
| ADR | Title | Status |
|-----|-------|--------|
| **0113** | Self-driving delivery loop — autonomous actuation, distributed & human-gated judgment (master as bounded coordinator) | **Accepted** (PR #423, 82509e0, FRE-828). Sensor→brain→hands; automate actuation, distribute+human-gate judgment; **sensitive-path merge carve-out + deploy human-stamp preserved**. Impl chain **FRE-829–835 Needs-Approval** (Build/ADR Dispatch Automation); **FRE-828 = seam umbrella** (Backlog — closes only on the assembled end-to-end demo). Supersedes ADR-0110's dispatch/poll half. |
