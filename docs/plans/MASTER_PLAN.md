...
- 🔴 **FRE-560** (Urgent) — consolidation-never-triggers — **in flight, occupies one build worktree**; gates FRE-523 KG-half. Excluded from dispatch.
- **FRE-523** — In Progress; resume KG verification + eval pass-2 when 560 ships (procedure in § Pending Verification).
...
**Worktree assignment now:** FRE-560 holds one build worktree → the free worktree starts **Lane A (544)**; when 560 lands, its worktree resumes **Lane B (545)**. A and B don't collide, so both lanes can run truly parallel whenever both worktrees are free.
