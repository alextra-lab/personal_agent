...
> **Last updated**: 2026-05-12 — FRE-328 shipped (PR #43): `missing_skill_requested` event on `read_skill` unknown-name; ES aggregation in `TelemetryQueries`; `InsightsEngine.detect_missing_skill_patterns` (≥3 requests across ≥2 sessions); flows through existing CaptainLog → promotion → Linear pipeline with stable per-skill-name fingerprint.
...
| **F** | Self-improvement | Partial | [FRE-328](https://linear.app/frenchforest/issue/FRE-328) ✅ · FRE-226 · FRE-234 | FRE-328 Phase 1+2 shipped 2026-05-12 (PR #43); FRE-226 needs FRE-227 |
...
| Item | Date | Summary |
|------|------|---------|
| **FRE-328: `missing_skill_requested` event + cluster→Linear pipeline** | 2026-05-12 | Phase 1: `read_skill` emits structured `missing_skill_requested` warning with `requested_name`, `trace_id`, `session_id`, `available_skills` when called with unknown name. Phase 2: `TelemetryQueries.get_missing_skill_buckets` aggregates by `requested_name.keyword` with cardinality sub-agg on `session_id.keyword`; `InsightsEngine.detect_missing_skill_patterns` emits `Insight(insight_type="missing_skill")` when count ≥3 across ≥2 distinct sessions; wired into `analyze_patterns`. Insight flows through existing `create_captain_log_proposals` → `CaptainLogManager.save_entry()` (fingerprint dedup) → `PromotionPipeline` → Linear ticket "Create skill: `<name>` — requested Nx across M sessions" in `Needs Approval` state. `normalise_insight_title` collapses digit runs, so seen_count increments rather than spawning duplicates. Category: `ARCHITECTURE`; Scope: `TOOLS`. 10 new tests + 1 augmented existing test. 324 tests pass in regression sweep. PR #43. |
