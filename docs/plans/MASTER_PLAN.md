...
- **⚠ Delivery automation PAUSED (2026-07-10, owner-directed).** Both `seshat-gating-watcher` + `seshat-dispatch-orchestrator` are **STOPPED (inactive)** — intentional, **NOT a failure** (don't alarm the actuation probe); still *enabled-at-boot*; owner decides re-enable. **While paused: nothing auto-launches, no PR auto-flags master — every gate is manual (`/master <PR#>`).** Model when live (unchanged): watcher triggers master (ability-not-obligation); CI-red → plain message to worker; bounce → master `send-keys` worker; **build dispatch = Linear labels**; dispatch NEXT-resolution via the external resolver (`next_resolver --eligible`, FRE-846). Full contract: `.claude/skills/lifecycle-rules.md`.
...
- **ADR-0114 associative-memory study — STARTED (priority-#2 decoupled side-study, owner-approved).** Chain FRE-838→843 (v0) + 853→856 (v1 arms D–G) all Approved. **FRE-838 (isolated substrate) merged, but its live corpus-load verification FAILED (study-Neo4j OOM'd on the real corpus)** — a fix PR is **inbound on cc-build** (study-Neo4j memory bump + runbook); **gate it MANUALLY (watcher paused)**. After merge, master re-runs the export (from primary; prod read-only) → FRE-838 Done → **FRE-839 dispatches**. 839–856 genuinely need the corpus, not just the merge. Open finding: fold the arm-B (dedup-only) recall number into FRE-843's continuation gate.
- **FRE-858 (Urgent, Needs Approval) — Memory-Recall backlog triage.** 30-Approved "phantom queue" (approval ≠ dispatch; much superseded by 0105/0106/0109/Voyage/OVH). Full keep/park/cancel table in the ticket. **Owner-led planning, do NOT dispatch;** 2 strategic calls pending (ADR-0098 shelve rec · ADR-0106 W2 dispatch-timing). Subsumes the old FRE-805/621 owner items.
- **build1/build2:** otherwise dry (FRE-724/820/595/844/846 shipped — see history). **FRE-739** (ADR-0107 assembled seam) Approved-but-unlabeled; **FRE-807** parked.
