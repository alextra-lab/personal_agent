...
- **build1: ADR-0102 vision docs/PDF chain** (FRE-684/T4 building; 682/683 merged; seam 689 master-run → T5–T9). **build2: ADR-0116 channel track** — FRE-871 (**AC-2 proven live**) + **FRE-872 Done** (consolidated T2–T4 end-to-end, PR #514 merged **dormant** — every seat stays send-keys, zero live behavior change; 873/874 canceled); build2 now **dry**. **FRE-875 (In Progress) — Phase A drift-guard merged** (PR #516: `topology.mode` is the single source of truth, launcher derives channel-wiring from it, `--channels` flag removed; inert — all seats send_keys). **Phase B = the live per-seat cutover**, ask-first deploy, **deferred to a fresh master** (retires the idle-scrape); runbook + prerequisites on the ticket. **adr: idle.** Parked-Approved (dispatch when a lane frees): FRE-858 recall keep-bugs (733/751/762/760/805/850) · **FRE-876** (config D4 check) · **FRE-867** · **FRE-807**.
...
- **ADR-0116 event-driven dispatch — IN PROGRESS 2026-07-13.** Capability-gateway + per-seat MCP-channel push replaces poll + send-keys + idle-scrape (Phase-1 delivery-swap; actuation-only boundary; master-alone-merge preserved). **FRE-852 (ADR) + FRE-871 Done — AC-2 PROVEN LIVE** (headless channel-push fires a turn in an idle seat; individual-account allowlist works after `claude plugin install`; Risk row 1 resolved; cutover needs `enableAllProjectMcpServers`). **FRE-872 Done** (consolidated end-to-end wiring; PR #514 merged **dormant** — code + per-AC unit proof, live AC-1/3/6 halves owned by the 875 seam) → **FRE-875 (In Progress)**: **Phase A drift-guard merged** (PR #516, `topology.mode` single source of truth; inert) · **Phase B = live per-seat cutover** (adr→build2→build1→master, prove AC-1/3/6 per seat, delete the idle-scrape last; ask-first deploy; **parked for a rested master** — prereqs on the ticket: orchestrator reload of the new launcher, `SESHAT_CHANNEL_SECRET` provisioning, live-seat plugin-install + managed-settings). 873/874 canceled (consolidated). ADR-0113/FRE-828 canceled (history).
