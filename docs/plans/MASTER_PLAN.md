...
- **ADR-0114 associative-memory study — STARTED (priority-#2 decoupled side-study, owner-approved).** Chain FRE-838→843 (v0) + 853→856 (v1 arms D–G) all Approved. **FRE-838 (isolated substrate) merged (PR #468 + OOM/runbook fix #471), but the corpus export has now failed master verification TWICE** — first the study-Neo4j OOM (fixed by #471), then a completed run revealed the export drops 36% of prod relationships (a hardcoded `PROD_RELATIONSHIP_TYPES` allowlist omits the entity-to-entity associative edges RELATED_TO/USES/PART_OF/SIMILAR_TO — the exact structure the study needs; see FRE-838 comment). **Fix #2 inbound on cc-build** (dynamic rel-type enumeration + exported==prod invariant test); watcher is live so it auto-flags master when green. After merge, master re-runs the export (from primary; prod read-only, provably untouched both failures) → FRE-838 Done → **FRE-839 dispatches**. 839–856 genuinely need the *complete* corpus. Open finding: fold the arm-B (dedup-only) recall number into FRE-843's continuation gate.
