...
- **FRE-545** ⏳ OPEN — deployed 2026-06-14 (PR #215, `37c4203`); ES mapping + `_field_caps` types verified, joinability green. **Remaining (read-only, next natural turn):** a turn-level `agent-topology-*` doc carries `model_role` (=`primary`) + populated `intent_confidence`/`decomposition_strategy`/`decomposition_reason`. Master will not fire a turn. On confirm → close Done.
