...
- **Taxonomy — entity *type* + knowledge *class* both LIVE.** ADR-0109 **V2 10-type** live end-to-end (audit-confirmed). **ADR-0115 (knowledge-class axis) Implemented + deployed 2026-07-12** (SHA `c51a7486`, migration 0019 as admin role): new entities persist `class` (World/Personal), System-natured items route to `sysgraph.stat` by absence-of-write, 180d session retention loop live. **Proven live** on a sanctioned turn (5 World entities classed vs pre-deploy 0/7992; 4 findings dispatched to sysgraph, 0 leaked to Core; relationships-skip working). FRE-863/864/728/860 **Done**. Supersedes ADR-0106 + ADR-0098-§D1; refines ADR-0097 (`System` → `output_kind`). **Existing-corpus de-noise DONE 2026-07-12** (owner-authorized ops, verified live): **FRE-865 backfill** classed all 7,992 → 6,215 World + 410 Personal + 1,372 System-marked (fail-open 3.2%, $0.31), then **FRE-868 eviction** removed those 1,372 from Core (1,014 findings→sysgraph, 358 ephemeral dropped, 126 MB snapshot retained on-box). Core now **6,625 entities, all World/Personal, 0 System** — **ADR-0114 existing-corpus de-confound complete**; both **Done**. **Gateway rebuilt to `7131c011` 2026-07-13** (owner-authorized) — shipped **FRE-869** cost-attribution fix (lanes corrected; confirms in next reconciliation window) + **FRE-632** owner-identity forward-fix; both **Done**. **FRE-632 owner-identity unification DEPLOYED + verified live**: the two split `Alex` nodes folded into one `:Person:Entity` (is_owner+embedding+all rels intact, 0 self-loops), forward-fix held across bootstrap (no re-fork) — "who am I" anchor now whole. ADR-0104 structural arm now **wired but flag-dark** (**FRE-866** Done 2026-07-13); flag-on rollout + class-bias policy deferred to owner.
...
- **build1: ADR-0102 vision docs/PDF chain** (T2–T9 FRE-682–689, fed in T-order, seam 689 master-run; T2/682 merged, T3/683 building). **build2: Memory-Recall queue** (owner-directed 2026-07-13, priority/leverage order) — FRE-866 arm-wiring Done → **FRE-632** owner-identity node-split building (re-tiered Opus: canonical-identity design call + prod migration) → then FRE-739 → recall keep-bugs. **adr: DRY.** Dispatchable-when-owner-sizes: **FRE-867** (watcher permission-surfacing) · **FRE-807** parked.
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — Approved + dispatched 2026-07-12** (owner-directed): FRE-682–689 Approved, head FRE-682 on build1, seam FRE-689 master-run. **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner accepts via `/adr` before dispatch).
2. **Memory Recall** — hot path. Taxonomy/extraction: entity-*type* ADR-0109 **V2 live end-to-end**; knowledge-*class* **ADR-0115 Implemented + live** (2026-07-12; existing-corpus de-noise **complete** — FRE-865/868 ran, Core class-clean at 6,625 all-World/Personal). Recall multipath: ADR-0103 Accepted / **ADR-0104 Partial** (2-arm live; structural/type arm coded-but-dark, unwired — FRE-866; blocked on class persistence — NOT Implemented).
