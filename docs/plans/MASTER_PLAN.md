...
1. FRE-552 ✅ (PR #209) → 2. FRE-547 ✅ cap-util ES snapshot (PR #210, deployed) → **3. FRE-543 ILM/retention ← next** → 4. FRE-544 dyn-field bound → 5. FRE-546 cost-cache Kibana import fix → 6. FRE-550 joinability breakdown panels → 7. FRE-477 discovery batching (Turn Cost) → 8. FRE-487 govern model file-reads (Turn Cost).
...
1. FRE-523 ✅ eval-mode memory bug (PR #208, deployed; pending eval-run AC) → 2. FRE-517 ✅ per-topology rows (PR #211, deployed) → **3. FRE-548 topology→ES projection emitter ← next** *(Telemetry ticket; here to avoid colliding with 517 on topology code)* → 4. **FRE-545 project route-trace-ledger routing fields → ES** *(re-anchored 2026-06-10: ADR-0088 ledger projection, NOT a new ADR-0082 event; reuses the 548 pattern)* → 5. FRE-522 eval⇄PWA → 6. FRE-541 conversation driver (unblocks 453; Tier-1 but no new ADR — amends taxonomy spec) → 7. FRE-551 artifact E2E extend.
