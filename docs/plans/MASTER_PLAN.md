...
> **Last updated**: 2026-06-07 (master) — **🔓 FRE-510 SHIPPED + DEPLOYED (PR #182, merge `26dfd9d`, deployed 15:21Z)** — artifact viewer iframe `sandbox=""` → `sandbox="allow-scripts"` (ADR-0089 D2/D3; supersedes ADR-0070 D7). Scripts run in a unique **opaque origin** — no PWA session/storage/cookies/parent-nav; `allow-same-origin` never granted (pinned by new vitest regression suite). Load-bearing boundary = **FRE-509 Worker CSP** (infra repo, Done 14:04Z, deployed *before* the flip — walls-before-scripts). SW shell **v20**. Master code+security reviews clean. Deploy verified: v20 SW + `sandbox:"allow-scripts"` in served chunk on :3000. **Ops note: PWA changes deploy via `SERVICE=seshat-pwa`, NOT the gateway.** Bounded open items: owner browser-checks live CSP header (VPS can't probe through CF Access); full script-artifact E2E waits on FRE-511 (sanitizer retirement — Approved, build may start; its deploy needed 510 live, now satisfied). **FRE-506 deployed 13:42Z → Done. L0: 451✅452✅513✅514✅519✅506✅; 505 deployed (E2E pending); 453 in progress (build2, PR #183 shipped); 515/517 Approved; 518 parked. ADR-0089: 509✅ 510✅ → 511 (build, next) → 512 (consumes 506 gate telemetry).** Previously: 2026-06-07 (master) — **👁️ FRE-506 SHIPPED (PR #181, `a9f87d2`) — **sandbox gate-decision telemetry** (L0). One `artifact_gate_decision` event per artifact commit: `enforced_pass`/`stripped`/`rejected`/`bypassed`/`not_applicable` + `commit_path`/`gate_ran`/script·handler·cdn counts; identity-threaded (trace/session/user). **Root cause confirmed:** the `87cbd720` incident committed `<script>` via the **ungated `artifact_write` direct path** (no HTML gate) after `artifact_draft` failed a malformation check — now emits `bypassed` instead of a silent commit. `artifact_write` stays **intentionally ungated** (owner-approved, ADR-0089 D1: served-CSP envelope is the boundary, not content inspection); visibility-only, removes/relaxes nothing. ES audit done (keyword/long via existing templates, no change). CI green (3356). **Scope honesty: durable/queryable telemetry, NOT live-meter** — surfacing gate decisions on the PWA meter needs routing through `report_degradation`→projector (ADR-0088 D5); separate follow-up or folds into FRE-512. **Deploy pending owner** (rebuild only; no migration; verify `artifact_gate_decision` fires on next artifact commit). FRE-506 → Done on verify. **L0: 451✅452✅513✅514✅519✅506✅(pending deploy); 505 deployed (E2E pending); 453/515/517 Approved; 518 parked.**
