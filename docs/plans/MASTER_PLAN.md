...
- **build1: ADR-0102 vision docs/PDF chain — SHIPPED + LIVE (2026-07-15). ADR-0102 Implemented.** Seam T9/689 AC-SEAM verified live: gateway rebuilt from `a43dc817` + PWA v31 (owner-authorized); native PDF document block on Sonnet (correct read, ~$0.045), cost gate ($0.5244 vs $0.50 held for confirm), vision path (Qwen), and joinability all green. All children T1–T8 Done. **Live gotcha:** the deployed local Qwen is vision-capable, so the AC-SEAM fail-closed leg is unreachable live (a Local override succeeds rather than failing closed) — proof stays the FRE-684 unit test. **Follow-up FRE-885 (Needs Approval):** cost-estimator token counter rejects the `document` content-block type → pre-flight token counts undercounted for document turns (telemetry-only; billed cost unaffected). **FRE-886 SHIPPED + LIVE (deployed 2026-07-15):** attachments now default to cloud/Sonnet (Auto → escalation model) for images+PDFs; $0.50 gate unchanged; reversible via `attachment_default_processing_target=local` + rebuild. **build1 idle.** **build2: FRE-884** — retire the orphaned ADR-0086 artifact-decomposition path (deploy ask-first).

- **NEXT EPIC — Seshat config-management interface (owner-directed 2026-07-15).** The capstone SURFACE on ADR-0099's single-source role matrix + validator. **cc-adrs designing the ADR now**, with the model-selection work (ADR-0118 / artifact-builder, FRE-878–883) folded in as its **first panel** — not shipped standalone. **Owner's binding constraint: keep it tight** — observe-first (read config first), model-selection + FRE-886 attachment-default as the only first panels, tier/profile routing (0082/0094/0095) explicitly deferred. This reframes the oversized "Seshat Inference Architecture" bucket (do NOT restructure it; the config interface is its surface). ADR-0118 approval HELD pending the ADR. **cc-explore deliberating** the multi-parent associative-memory question (does it earn its keep) via the flag-dark ADR-0104 structural-arm live A/B — read-only, owner-gated. Parked-Approved: FRE-858 keep-bugs · **FRE-876** · **FRE-867** · **FRE-807**.
...
- **ADR-0116 event-driven dispatch — Phase 1 LIVE (channel cutover 2026-07-14).** Per-seat MCP-channel push replaced poll + send-keys idle-scrape; all 3 worker seats channel-mode, master stays send-keys. **FRE-852/871/872 Done.** **FRE-875 PARKED (Approved, owner 2026-07-15)** — the two remaining seam items are deferred hardening, non-blocking: (1) seat-secret durability (secret in tmux global env, lost on box restart → degrades safely to send-keys); (2) scrape deletion (still the channel-down fallback + master nudge gate; re-home before removing). 873/874 canceled; ADR-0113/FRE-828 canceled (history).
- **Master tooling shipped this session (ADR-0117 Implemented, FRE-877 Done).** `scripts/pr_gate.py` — the deterministic **signal collector** at master SKILL Step 4 (determinable CI/mergeability/dependabot facts only, never judges/blocks; the owner's anti-caging constraint — judgment stays master's). Trust boundary codified in `lifecycle-rules.md § Signal trust boundary`. **ADR index guard** (`scripts/next_adr.py --next/--check` + pre-commit) — authoritative ADR numbering + index-completeness, closing the double-number collision + index-drift both seen this session. **ADR-0118** (model-selection layer / user-selectable artifact builder, Phase-1) = **Proposed** — explore's Stream-2 seed (WIP, post-vision).
...
**Sequencing rationale (current):** build1 = ADR-0102 vision chain — SHIPPED + LIVE (T1–T9 all Done, verified 2026-07-15); now on **FRE-886** (default Auto attachments → cloud/Sonnet, owner-requested). build2 = FRE-884 (ADR-0086 retirement).
ADR-0116 dispatch = Phase-1 LIVE (FRE-872 Done; FRE-875 seam parked). After build1 frees from FRE-886: the parked
FRE-858 recall keep-bugs + FRE-876. FRE-739 (Awaiting Deploy, ADR-0107 seam — assembled verify needs FRE-740 + a
live non-owner request). FRE-717 held at Awaiting Deploy for organic outcome input.
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — SHIPPED + LIVE (2026-07-15), Implemented**: all children + seam FRE-689 Done, AC-SEAM verified live (native PDF block on Sonnet, cost gate, vision path, joinability green). Follow-ups: **FRE-885** (Needs Approval — cost-estimator token-counter document-type gap, telemetry-only) · **FRE-886** (Approved, building — default Auto attachments → cloud/Sonnet). **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner accepts via `/adr` before dispatch).
