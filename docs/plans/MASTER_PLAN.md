...
> **Streams next:** **build → FRE-488 then FRE-489** (recall harness scaffold → probe set; advances FRE-435; both in flight in `build` worktree) · **build2 → FRE-339 then FRE-395** (PWA runtime-config.json → ESLint; FRE-339 in flight in `build2`) · **adr → anomaly-triage DONE (PR #255 merged)**; idle until owner greenlights the self-diagnosing `/adr` cycle the brief seeds (or feed it FRE-345/259). Also-Approved backlog for build: 619 · 605 · 593 · 585. Live queues below: § Immediately Actionable · § Needs Approval · § Active ADRs.
