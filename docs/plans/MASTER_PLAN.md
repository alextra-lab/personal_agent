...
| [FRE-498](https://linear.app/frenchforest/issue/FRE-498) | — | Sonnet | **Measure mermaid-CDN reach** in artifact drafts (post-ADR-0089: scripts allowed but `connect-src 'none'`+host-allowlist blocks external CDN → such artifacts silently don't render; portability-quality signal). Candidate to fold into FRE-453 eval coverage. Project: *Artifact Execution Security*. |
