...
Master-maintained dispatch. `/build 1`, `/build 2`, and `/adr` resolve their NEXT ticket **and** the context action here. Master advances this at each merge.

<!-- STREAM-BOARD:START -->
| Stream | Command | NEXT (build this) | Context | Queued after |
|--------|---------|-------------------|---------|--------------|
| build · Stream 1 · **Opus** _(flip to Opus for 724)_ | `/build 1` | **FRE-724** [O] _(recall v1 core SEAM — assemble arms→RRF→rerank into MEMORY_RECALL behind the flag; prove ADR-0104 AC-1/3/5/6 live. All deps met: 722✅ 723✅ 706✅)_ | CLEAR | after **724** (recall live) → **build1 Opus queue** (priority plan): 704 → 658 → 432-Ph0 → 725 → 630 → 697/699 → 472. _(ADR-0104 Implemented on live proof; master gates the flag rollout)_ |
| build2 · Stream 2 · **Sonnet** | `/build 2` | **FRE-693** [S] _(ADR-0074 joinability for the vision/attachment path; 692 shipped+deployed)_ | **KEEP** _(same attachment/vision area per 692 handoff)_ | vision cleanups 726[S]/735[S]/668[H]/742[H] → **master: run FRE-669 seam + close FRE-691 cloud-cost AC live = ADR-0101 DONE** → **then priority waves (Sonnet)**: W0 stability (Config 648→649 · Memory 659/620/632/676/657/677 · Linear 710) → W1 substrate (Config 650→652+735 · Memory 639/640/641/642 · Linear 720/714/715/716) → W2 boundary (728/730→731→732→729 · 738) → W3 (Pedagogical 561→564→453 · Linear 719/718 · Memory 660/647). _(ADR-0102 doc chain 682–689 **owner-PAUSED** — do not build)_ |
| adr · **Opus** | `/adr` | **FRE-736** [O] _(stored-image re-vision ADR — **Approved**; extends ADR-0101 to stored artifacts via credentialed re-fetch)_ | CLEAR | ADR-0094/0095 accept-or-kill (after Inference 432/516 measure) · FRE-565 past-conversations ADR · pedagogical-reflection ADR (unfiled). _(639↔0105 reconciliation = **resolved** by ADR-0106, Accepted)_ |
<!-- STREAM-BOARD:END -->

**Context flag:** default **CLEAR** (fresh context per ticket). **KEEP** is set only when the NEXT ticket is a direct follow-on to what the stream just shipped (same files/substrate) — then run the command on the warm context, do **not** `/clear`. **Parked / held (not auto-dispatched):** ADR-0102 doc chain FRE-682–689 (owner-paused) · Inference ADR-0094/0095 trees FRE-600–604 / 607–611 (held pending 432/516 measurement) · FRE-713 (trigger-gated, Backlog). **Full cross-project order → [`2026-07-02-priority-sequencing.md`](2026-07-02-priority-sequencing.md)** — Waves 0 stability → 1 substrate → 2 System-boundary → 3 measurement → 4 held. WIP = 1 live ticket per stream.

**§ Owner reassessment (2026-07-01) — RESOLVED, streams re-homed.** (1) **Recall v1 core FRE-722/723/724 Approved** (owner, 2026-07-01). Homing (routing-tier-clean): **722→723 (Sonnet) on build2 after the vision chain** (709→666); **724 (Opus seam) on build1** after 707 + 722/723/706; **707 (Opus structural arm) on build1 now**. *Priority note:* the v1 core (722/723) is the lived-symptom fix but is Sonnet, so it queues behind the near-done vision chain on build2 — if the owner wants it to start immediately, either pause vision or run 722 on build1 (Opus-on-Sonnet). (2) **FRE-639 hold = gate-vs-isolate for the System class.** FRE-639 (ADR-0098 T3) *gates* System-subject extracted entities (the ~46% dev-phase operational noise: healthchecks, log review, harness explainers) — classify + exclude from recall + evict — but keeps them **in** Core (a query-time class filter). FRE-708/**ADR-0105 (now Done, PR #304/#305)** argues the opposite for System data: *physically isolate* it into a separate graph ("a query-time class filter can be forgotten"), explicitly refining ADR-0098 and flagging "coordinate the isolation call with FRE-639." **The blocker (ADR-0105) now exists** → the reconciliation is makeable: does 0105's isolate-don't-gate ruling extend to 639's conversation-extracted System entities (→ re-scope 639 toward isolate), or are they distinct scopes (self-improvement pipeline data vs conversation entities) where 639's gate stays right? **This reconciliation is fresh adr-session design work** (queued after FRE-706) — which also refills the adr stream. (3) adr no longer dry: FRE-706 sign-off → the 639↔0105 reconciliation.

> **Last updated**: 2026-07-02 (master; **FRE-692 shipped+deployed** #329 — per-attachment Cloud/Local override live, PWA CACHE_NAME v29, ticket Done on narrow AC; **FRE-691 gateway rebuilt live** (image `9bb0f43`→current, pricing registered count=2, cloud vision now metered non-zero — AC-11 core proven live; **AC-10 pre-flight estimator/threshold path still unproven live**, needs a forced-Cloud image turn now that 692 is live → FRE-691 stays In Review, one leg of the open FRE-669 seam); ADR-0107 merged #327; **6-project priority-sequencing plan** → [`2026-07-02-priority-sequencing.md`](2026-07-02-priority-sequencing.md) — 24-ticket approve-list + cross-project waves, owner sign-off pending). **Current state:** the **🎛️ Stream Board above is live** — 4 streams building via `/build 1` `/build 2` `/adr` (board resolves ticket + KEEP/CLEAR context). **✅ VISION+MEMORY DEPLOYED 2026-07-02** (latest gateway `9bb0f43` — **FRE-734 vision config-parity fix redeployed 12:24Z**; health green · corpus intact · flags inert · **vision routing FIXED live** — all 4 models `supports_vision=True` in-container, startup guard logs `vision_capabilities_at_startup` + warns on drift, zero `AttachmentUnsupportedError` since rebuild. Root cause was the gateway loading `models.cloud.yaml` (not `models.yaml`) — FRE-665 flag was only in the latter. **VISION CORE CONFIRMED LIVE** (owner-tested 12:55, session 85e66b60 — fresh attach → local qwen3.6 → excellent image-conditioned description, no AttachmentUnsupportedError; **FRE-734 Done**). **FRE-669 stays OPEN** — its FRE-690-amended AC-SEAM (override AC-9 · cost AC-10/11 · joinability AC-12) awaits the unbuilt FRE-691/692/693; do NOT close the seam on the image-seeing core. Memory first-Claims inspection on next consolidation). Now-live chains: **vision** (FRE-661+664+665+709+**666**, code-complete on the agent side — 666 = image resolution/block injection; **DEPLOY CHECKPOINT REACHED**, runbook drafted [`2026-07-02-staged-vision-memory-deploy-runbook.md`](2026-07-02-staged-vision-memory-deploy-runbook.md); deploy ask pending owner. Local-only in practice — cloud spine 691/692/693 + seam 669 unbuilt) + **flag-dark** FRE-707 (structural recall arm — inert until FRE-724 flips the flag) + **memory** (FRE-637+638+712+711, new Neo4j Claims write-path; 711 = World-fact living descriptions — **correction path DORMANT with current callers** (all conversation sources = 0.8 baseline → strict-`>` never fires; only empty-fill live), FRE-375 preserved, real corrections await FRE-725 higher-confidence extractor signal — **owner decision at deploy: pull FRE-725 before the memory deploy, or ship D2 as mechanism+empty-fill?**). **Deployed via the staged runbook** (`2026-07-02-staged-vision-memory-deploy-runbook.md`) — Neo4j snapshot taken (`.backups/neo4j-predeploy-2026-07-02.tgz`), rollback image `12cf0912`. Cloud vision spine (FRE-691/692/693) + seam FRE-669 still unbuilt → **vision is LOCAL-ONLY** (Qwen) until 692 ships the cloud override; **memory Claims write-path is live but idle** (writes on the next consolidation — inspect first Claims then). FRE-725 approved (correction enrichment) — corrections stay empty-fill-only until it ships. **Poll:** master runs a self-paced /loop gating PRs ~20 min; the wakeup is harness-scheduled and survives /clear+re-prime — re-primed master: confirm it's armed, re-arm if not. **Owner-gated (not auto-dispatched):** FRE-704 (ES field-drop — master brings raise-cap-vs-prune approach). [FRE-711 approved+dispatched to build 2026-07-01, KEEP context]. **Shipped this session** (all gated + verified live where deployed): FRE-637 · FRE-661 · FRE-664 · FRE-665 (ADR-0101 vision routing — supports_vision flag + capability-driven routing, PR #306, code-only under deploy-hold) · FRE-638 · FRE-712 (ADR-0098 D2 memory: retire-FWW + facet-key supersession) · FRE-711 (ADR-0098 D2: World-fact living descriptions, PR #308, code-only under deploy-hold — correction path dormant until FRE-725, see deploy-hold note) · FRE-494 (ADR-0103 Accepted + ADR-0104 Proposed, recall multi-path) · FRE-705 (ADR-0104 multi-path retrieval design spec, PR #307; build chain FRE-722/723/724 **Approved**) · FRE-706 (recall operating-point decision — noise-guard floor **0.60**, owner-signed-off, PR #317; applied+proven live by FRE-724) · FRE-727 (**ADR-0106 Accepted** — System/User boundary by output_kind write-time dispatch, no reasoning cage; reconciles/supersedes FRE-639, PR #324; impl chain FRE-728–732 Needs-Approval; seam AC-1+2+3 master-held) · kibana loop dashboards #292/#296/#297/#301/#302 + delegation-outcomes retired; + **FRE-707** (ADR-0104 structural recall arm, PR #321 — flag-dark, review-proved flag-off recall byte-identical; activates via FRE-724) · **FRE-722** (ADR-0104 RRF fusion combiner + dedup, PR #322 — pure module, dead code until FRE-724) · **FRE-723** (ADR-0104 lexical full-text + multi-query arms, PR #325 — flag-dark; `turn_entity_fulltext` index creates on next gateway rebuild; re-gated after a master bounce for the multi_query session-wrap) · **FRE-709** (ADR-0101 §2 content-stringify hardening, PR #316 — budget.py counts image blocks; re-gated after a master bounce) · **FRE-703 dashboard-redesign wave #309–320** (10 dashboards: cost_budget · llm_performance · traversal_gate · expansion_decomposition · intent_classification · task_analytics · extraction_retry_health · request_timing · request_traces · system_health — UI-built Lens, imported to live Kibana + cache-busted render-verified: zero render-traps, real-data story confirmed; #315 also fixed a fictitious-`dead_letter` mislabel + an aggregate-misleads trap; FRE-703 loop continues); + **Stream Board workflow** + `create-visualization` skill. · **ADR-0107** (user identity for Claims + log/trace propagation — **Accepted**, PR #327 merged; amends ADR-0098 D2 Claims-only, Stance unchanged; impl children FRE-738/739/740 **Approved**; ⚠️ owner to run the one-time Susan-Claim Cypher re-point per ADR-0107 Impl Notes). · **FRE-691** (cloud attachment cost controls — pricing + pre-flight $0.50 threshold + ADR-0065 reservation/metering, ADR-0101 §8b; PR #328 merged, **code-only under deploy-hold** — gateway rebuild + threshold-confirm pending owner; stays In Progress until deployed+verified; one leg of the FRE-669 seam). **Approved & queued:** FRE-709→666→**722→723** (build2) · **707** now + **724** after (build1) · FRE-706 sign-off → **639↔ADR-0105 reconciliation** (adr) · FRE-710/713. **Recall v1 core FRE-722/723/724 Approved** (owner 2026-07-01), homed per § owner reassessment. **Active pillars:** Memory Recall Quality (ADR-0087/0098/0100/0103/0104, parent FRE-435) · Agent Vision & Attachments (ADR-0101/0102) · Telemetry Surface Audit (ADR-0090) · Self-improvement loop / System-KG isolation (FRE-708). **Full session-by-session narrative archived → [`completed/2026-07-01-master-header-archive.md`](completed/2026-07-01-master-header-archive.md)** (prior: `completed/2026-06-26-master-header-archive.md`); `git log` for per-PR detail. This header is current-state only; sections below are the live plan.)
>
