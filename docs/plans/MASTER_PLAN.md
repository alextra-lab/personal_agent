...
| [FRE-473](https://linear.app/frenchforest/issue/FRE-473) | **High** | Sonnet | **Copy-isolation**: deepcopy at `respond()` boundary so provider decoration never mutates caller-owned/persisted session messages; contract test for ≤4 blocks post-LiteLLM-transform. Codex finding from FRE-468 review. |
