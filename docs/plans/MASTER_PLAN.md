...
- **Taxonomy — entity *type* + knowledge *class* both LIVE.** ADR-0109 **V2 10-type** live end-to-end (audit-confirmed). **ADR-0115 (knowledge-class axis) Implemented + deployed 2026-07-12** (SHA `c51a7486`, migration 0019 as admin role): new entities persist `class` (World/Personal), System-natured items route to `sysgraph.stat` by absence-of-write, 180d session retention loop live. **Proven live** on a sanctioned turn (5 World entities classed vs pre-deploy 0/7992; 4 findings dispatched to sysgraph, 0 leaked to Core; relationships-skip working). FRE-863/864/728/860 **Done**. Supersedes ADR-0106 + ADR-0098-§D1; refines ADR-0097 (`System` → `output_kind`). **Existing corpus DE-NOISED 2026-07-12** (owner-authorized ops, verified live): **FRE-865 backfill** classed all 7,992 `class=None` entities → 6,215 World + 410 Personal + 1,372 System marked (fail-open 3.2%, $0.31); **FRE-868 eviction** removed those 1,372 from Core (1,014 findings→sysgraph, 358 ephemeral dropped, snapshot-backed). Core now **6,625 entities, all World/Personal, 0 System** — ADR-0114 existing-corpus de-confound complete; both **Done**. ADR-0104 structural arm still dark (**FRE-866**, gated). **FRE-869** (cost-attribution fix — entity_extraction was billing `main_inference`, cap unenforced) **merged, Awaiting Deploy** (gateway rebuild, ask-first).
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — Approved + dispatched 2026-07-12** (owner-directed): T2–T9 (FRE-682–689) Approved, head **FRE-682 on build1**, chain fed in T-order, seam FRE-689 master-run. **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner to accept via `/adr` before dispatch).
