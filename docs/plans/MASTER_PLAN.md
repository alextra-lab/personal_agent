...
- ✅ **DEPLOY-GATE CLEARED (2026-06-28):** migration 0012 (`upload_pending` column) applied to prod Postgres — column present, 57 existing rows default FALSE. **FRE-369 DEPLOYED** (owner-approved): gateway + PWA rebuilt cloud-side, both healthy (presign route mounted 401, PWA serving 200), migration-first sequence. **FRE-369 still In Progress** pending the owner's live smoke test (attach a file in PWA → agent reads it); closes Done on smoke-test pass. Gateway is now safely deployable for the future FRE-655 recall rollout.
