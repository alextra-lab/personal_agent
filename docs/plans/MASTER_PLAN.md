...
- **Delivery automation — currently HALTED (2026-07-14, owner-directed, house-in-order before re-enable).** Both `seshat-gating-watcher` + `seshat-dispatch-orchestrator` are **stopped** with the `telemetry/dispatch.disabled` kill-switch set. **The 3 worker seats are now cut over to MCP-channel delivery** (ADR-0116 Phase B — adr/build2/build1 channel-wired + verified live; master stays send-keys); the channel is proven but not fed until the loop resumes. Go-live (resume) pending. When live, the model is: watcher triggers master when a PR is CI-green (ability-not-obligation — master leads "Gating PR #X"); CI-red → channel push (or send-keys fallback) to the owning worker; **build dispatch = Linear labels**; NEXT-resolution via `next_resolver --eligible` (FRE-846). Full contract: `.claude/skills/lifecycle-rules.md`.
...
- **build1: ADR-0102 vision docs/PDF chain** (FRE-684/T4 **merged** 2026-07-14, Awaiting Deploy/inert-baking; next = T5/FRE-686 Approved-but-unlabeled; 682/683 merged; seam 689 master-run → T5–T9). **build2: dry** (ADR-0116 channel track — FRE-871/872 Done; **FRE-875 Phase B DONE** — all 3 worker seats cut over; see the ADR-0116 thread below). **adr: idle** (FRE-877 ADR-first queued — see below). Parked-Approved (dispatch when a lane frees): FRE-858 recall keep-bugs (733/751/762/760/805/850) · **FRE-876** (config D4 check) · **FRE-867** · **FRE-807**.
...
- **ADR-0116 event-driven dispatch — IN PROGRESS (Phase B cutover DONE 2026-07-14).** Per-seat MCP-channel push replaces poll + send-keys + idle-scrape (Phase-1 delivery-swap; actuation-only; master-alone-merge preserved). **FRE-852/871/872 Done.** **FRE-875 (In Progress) = the seam:** Phase A drift-guard merged (PR #516) · **Phase B — all 3 worker seats (adr/build2/build1) cut over to channel-mode** (PRs #519 merged; box setup done: plugin install + managed-settings + shared secret; each seat verified live — channel registers, authorized POST → 200 fires a turn, wrong secret → 403). Master stays send-keys (no topology entry; scrape-free). **Remaining before the seam closes:** (1) **go-live** — resume the halted dispatch loop; (2) **secret durability** — the seat-side secret lives in the tmux global env, lost on a tmux/box restart (degrades safely to send-keys) → persist it (small launcher change); (3) **scrape deletion** — the ADR's final AC, a separate careful change (the scrape is still the channel-down fallback idle-gate + the master context-pressure nudge's gate; must re-home both, not just delete). 873/874 canceled. ADR-0113/FRE-828 canceled (history).
- **FRE-877 — deterministic PR-gate preflight + handoff template (Needs Approval, ADR-first).** Owner-requested this session: the gating twin of `reconcile_board.py` — a no-LLM preflight that verifies every checkable signal (CI, PR hygiene, ticket state, handoff completeness, codex mis-tier) and fails loud, leaving master only the semantic residue. Spec: `docs/specs/PR_GATE_DETERMINISTIC_PREFLIGHT.md`. Owner chose ADR-first (routes to the adr session). The trust boundary this operationalizes is now codified in `lifecycle-rules.md § Signal trust boundary`.
...
**Sequencing rationale (current):** build1 = ADR-0102 vision chain (T4/684 building → T5–T9, seam 689 master-run).
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — building** (owner-directed): FRE-682/683 merged, T4/684 on build1, seam FRE-689 master-run. **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner accepts via `/adr` before dispatch).
