...
## Recent decisions (last session — 2026-07-08, later)
*Rolling bridge for the next session: decisions + why, not a work log (Linear has that). `/prepare-reset` replaces this each reset.*

- **DISPATCH AUTOMATION NOW FULLY LIVE END-TO-END (do NOT re-open).** The orchestrator was **installed + enabled as a systemd daemon** (`seshat-dispatch-orchestrator.service`). Root cause it "never worked / was removed": the committed unit had **no `PATH`** → `claude` not found → fail-to-start → removed. Fixed (PR #445 `Environment=PATH`). Launcher slot-collision fixed (PR #448 — kill+recreate the *persistent* tmux slot instead of `new-session` colliding). Watcher now **always sends `/master <id>`** (PR #447 — idle-gate was a fragile screen-scrape that kept a busy master uninformed; workers keep the idle guard). **Proven: orchestrator auto-launched FRE-820 `launched=True`.** ⇒ **Build dispatch = Linear labels now** — labeling an Approved+unblocked ticket on a free stream auto-launches within a poll tick; send-keys `/build` retired. See [[project_dispatch_orchestrator_live]].
- **Shipped + deployed + Done:** **FRE-710** (Captain's-Log reflection → per-session cadence) · **FRE-718** (Postgres RAM/CPU tuning: cpus 1.0, right-sized cache, pg_stat_statements + a scheduled sysgraph vacuum/analyze job) — both AC proven live.
- **Reranker → Voyage (dispatched FRE-851, Approved, stream:build1).** Voyage rerank-2.5 **primary** + Mac-tunnel 4B **fallback**. Latency proven **in-pipeline** ~250ms vs 4B ~2.5s (~10×); FRE-695 quality-equivalent (J 0.73≈0.747); **OVH has no reranker**. Reranker = **latency/reliability fix, NOT a recall-ceiling fix** (no reranker opens a clean floor — structure does, ADR-0103). See [[project_reranker_voyage_direction]].
- **Accurate multipath test STILL not obtained — now blocked by the Mac sub_agent flaking** (multi-query paraphrase LLM timed out under load: 10s / 429 / 503 → `paraphrase_count=0`). **Both** Mac-tunnel deps (reranker AND sub_agent) need managed endpoints; a clean 3-arm run can't complete until the sub_agent is managed too. Setup + gotchas: [[reference_accurate_multipath_test_setup]].
- **`localhost:8000` dead SLM endpoint retired → tunnel (PR #451, merging).** That dead local port is why the FRE-778 harness couldn't reach the sub_agent.
- **#449 (FRE-778 instrument) PENDING owner decision: bounce for fail-loud** — it silently degrades when an arm can't run (produced 2 bad tests this session). Recommended bounce; not yet confirmed.
- **Correction (owner, hard, saved to memory):** a **bad eval is discarded, not reframed** as a "lower bound"; **check stored evals/research/config BEFORE running anything (esp. paid)** — FRE-695 already held the Voyage number I redundantly re-probed. Verify before acting; own it, don't spin.
- **MASTER_PLAN header (line 6) is heavily bloated — flag: do a DEEP compaction (move 07-06/07-07 completed narrative to MASTER_PLAN_HISTORY) as an early fresh-session task.** This checkpoint did the *safe* pass only (block + this note), not a header restructure.
