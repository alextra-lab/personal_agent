...
**Sequencing rationale (current):** build1 = ADR-0102 vision chain (T4–T7 merged/inert-baking → T8/687 building → seam 689 master-run).
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — building** (owner-directed): T1–T7 merged (inert-baking), T8/687 on build1, seam FRE-689 master-run. **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner accepts via `/adr` before dispatch).
