...
| Ticket | Priority | Tier | What |
|--------|----------|------|------|
| [FRE-525](https://linear.app/frenchforest/issue/FRE-525) | — | Opus | **Curated artifact toolkit** (the substantive remaining artifact work): research → decide the `/lib/` library set (KaTeX/charting/Tailwind/…) → host version-pinned on the Worker → reframe `_HTML_GENERATION_SYSTEM_PROMPT` + author a frontend-design skill with per-library best practice. ADR-0089 D2a; makes the sealed box *useful*, not just safe. **Supersedes FRE-498.** Project: *Artifact Execution Security*. |
| [FRE-524](https://linear.app/frenchforest/issue/FRE-524) | — | Sonnet | **Worker-side per-serve envelope logging** for artifacts.frenchforet.com (FRE-512 covers per-commit probes only; browser serves never traverse this repo). Lives in `personal_agent_secrets` Worker. Project: *Artifact Execution Security*. |
| [FRE-498](https://linear.app/frenchforest/issue/FRE-498) | — | Sonnet | **Mermaid-CDN reach measurement — OBSOLETE as written** (measures `_sanitize_sandbox_violations`, deleted by FRE-511). Live portability question folded into **FRE-525** Phase-1 research. → **Cancel on FRE-525 approval.** |
| [FRE-497](https://linear.app/frenchforest/issue/FRE-497) | — | Opus | **Self-correcting deterministic gates** — feed validation failure back to the model for N bounded retries before hard-fail. Post-FRE-511 only `_validate_html_output` (malformation) remains in the artifact path; principle is system-wide → consider re-home to *Inference Architecture* / *Turn Reliability*. Project: *Artifact Execution Security*. |
