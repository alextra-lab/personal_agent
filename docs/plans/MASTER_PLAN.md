...
**Recommended order**: ADR-0081 D4 (skill-index split — unblocks the FRE-422 cache gate) → FRE-377 (quick canonicalization) → FRE-408 (P4) → FRE-399 Layer 2 → FRE-400 → FRE-397 Tier 2.
...
| ADR | Title | Status |
|-----|-------|--------|
| **0081** | **Cache-Aware Context Layout & Compaction** | **D1 Implemented 2026-06-01 (FRE-422, PR #120, deployed); cache gate blocked on D4 (skill-index split); D2/D3/D5/D6 pending. ADR status field still `Proposed` — adr session to bump in the D4 PR.** |
