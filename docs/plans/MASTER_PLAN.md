...
- **Taxonomy — entity *type* + knowledge *class* both LIVE.** ADR-0109 **V2 10-type** live end-to-end (audit-confirmed). **ADR-0115 (knowledge-class axis) Implemented + deployed 2026-07-12** (SHA `c51a7486`, migration 0019 as admin role): new entities persist `class` (World/Personal), System-natured items route to `sysgraph.stat` by absence-of-write, 180d session retention loop live. **Proven live** on a sanctioned turn (5 World entities classed vs pre-deploy 0/7992; 4 findings dispatched to sysgraph, 0 leaked to Core; relationships-skip working). FRE-863/864/728/860 **Done**. Supersedes ADR-0106 + ADR-0098-§D1; refines ADR-0097 (`System` → `output_kind`). **Existing-corpus de-noise DONE 2026-07-12** (owner-authorized ops, verified live): **FRE-865 backfill** classed all 7,992 → 6,215 World + 410 Personal + 1,372 System-marked (fail-open 3.2%, $0.31), then **FRE-868 eviction** removed those 1,372 from Core (1,014 findings→sysgraph, 358 ephemeral dropped, 126 MB snapshot retained on-box). Core now **6,625 entities, all World/Personal, 0 System** — **ADR-0114 existing-corpus de-confound complete**; both **Done**. Remaining ops decision: **FRE-869 gateway deploy** (fixes live `main_inference` cost mis-billing — merged, Awaiting Deploy, ask-first, independent). ADR-0104 structural arm now **wired but flag-dark** (**FRE-866** Done 2026-07-13); flag-on rollout + class-bias policy deferred to owner.
