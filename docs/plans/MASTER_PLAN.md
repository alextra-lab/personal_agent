...
> **Last updated**: 2026-05-13 — FRE-337 skill nudge injection shipped (PR #48); deterministic XML directive blocks now appended after skill content to close the "model ignores loaded skill" behavioral gap. Two-sided eval (Family A ambiguous + negative-control) pending against live gateway.
...
| Item | Date | Summary |
|------|------|---------|
| **FRE-337: Skill nudge injection (ADR-0067)** | 2026-05-13 | Two deterministic XML directive blocks (`<skill_index_directive>` and `<skill_usage_directives>`) appended after all skill content in the system prompt, immediately before the user message. Closes the behavioral gap where the primary model (recall=1.0 from router) answered from training-data priors instead of executing the injected skill. `nudge:` YAML frontmatter field added to `SkillDoc`; seeded on 4 skills (query-elasticsearch, system-metrics, infrastructure-health, self-telemetry). Feature-gated via `AGENT_SKILL_NUDGE_ENABLED` (default `True`) for A/B eval. ADR-0067 accepted. 20 new tests, 0 regressions. PR #48. Two-sided eval (Family A + negative-control) pending. |
