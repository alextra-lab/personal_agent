...
> **Last updated**: 2026-07-05 (ADR-0110 dispatch automation T1–T4 shipped + live-verified; FRE-620 quality-monitor deployed). **ADR-0109 V2 taxonomy cutover SHIPPED end-to-end this session:** the 10-type extractor (FRE-771) + recall-consumer remap (FRE-794) are live in the gateway (rebuilt 09:54 from main), and the FRE-772 KG migration ran against prod Neo4j — **0 Technology/Topic/Concept remnants, joinability probe green** — batched + cost-gated via FRE-800/FRE-801 (~$0.43, ~38× fewer calls). **Correction:** the prior `0f46a84`-deployed note was inaccurate (baked from a stale tree still serving V1); the real V2 deploy is this session's rebuild. Also shipped this session: **FRE-796** web-tools fix (Perplexity→`sonar-reasoning-pro` + searxng chefkoch category-leak) live · **FRE-798** ruff-hook `--unfixable F401` · **FRE-677/795** test-hygiene · **FRE-790** boundary probe (κ 0.858) · **FRE-797 = no-change** (boundary clears; residual is a cross-provider artifact a definition can't fix). ADR-0099 config management **Implemented** (FRE-648–652 Done). **ADR-0110 dispatch automation — T1–T4 all shipped + live-verified this session:** FRE-785 resolver, FRE-786 (RC spike proven live owner-present + launcher), FRE-787 orchestrator loop, FRE-804 (live `fetch_board` GraphQL 400 fix — found at the FRE-787 gate; T1 had been merged on mocked tests, never live-exercised), FRE-788 (T4 ops: systemd units + RC-liveness guard + kill-switch + runbook; `--preflight`/kill-switch verified live) all Done. Remaining: **FRE-806** (prime-worker refactor — shed resolution, widen self-fix to {bounce, CI-red}, launcher seeds the ticket; build1, prerequisite) → then the **master-owned live assembled-seam demo** (install units + one monitored dispatch per stream + two-worker pytest-lock collision). **FRE-620** KGQ quality-monitor fix (`:Turn` + recalibration + promotion floor) merged **and deployed** (gateway rebuilt ~16:12Z; false daily high-sev anomaly killed, freshness revived). FRE-699 recall/rerank research shipped (PR #383, Done). **Dispatch now:** build1→**FRE-806** (prime-worker refactor, Opus/keep) · build2→**idle** (queue drained; FRE-803 pending approve) · adr **idle**. **Open owner items:** FRE-803 (init.sql `upload_pending` parity — one-line DR-rebuild fix, Needs Approval → build2 Urgent/Haiku) · FRE-805 (sibling stale `:Conversation` in a manual script, Needs Approval, Low) · FRE-621 (graph-hygiene cleanup, Needs Approval, Medium) · **MedicalCondition** entity-type seed (health-domain, parked) · config-UI capstone. Process-v2 baseline: Linear-native dispatch `stream:*`+priority+blocked-by; PR-merge→Awaiting Deploy. Prior narrative → [`completed/2026-07-04-master-plan-archive.md`](completed/2026-07-04-master-plan-archive.md).
