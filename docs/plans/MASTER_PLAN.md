...
> **Last updated**: 2026-07-17 (FRE-896 config-cleanup chain SHIPPED end-to-end — 876/896/897/906/907; FRE-906 event-bus self-reclaim DEPLOYED + live-verified; ADR-0120 cost-gov Proposed; backlog reel-in done; tmux 3.7a cutover pending. See LAST_SESSION ▶ THIS RESET IS THE TMUX CUTOVER)
...
- **tmux:** server cut over to **3.7a** (built from source, `/usr/local/bin`; apt 3.5a fallback in `/usr/bin`) — fixes the control-client crash class that killed `cc-build2` mid-build 2026-07-16. Recovery runbook: memory `reference_worker_seat_crash_recovery`.
