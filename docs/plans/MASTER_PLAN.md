...
- **build1: ADR-0102 vision docs/PDF chain** (FRE-684/T4 building; 682/683 merged; seam 689 master-run → T5–T9). **build2: dry** (ADR-0116 channel track — FRE-871/872 Done; **FRE-875 Phase B** = the live cutover, deferred to a fresh master; see the ADR-0116 thread below). **adr: idle.** Parked-Approved (dispatch when a lane frees): FRE-858 recall keep-bugs (733/751/762/760/805/850) · **FRE-876** (config D4 check) · **FRE-867** · **FRE-807**.
...
- **ADR-0116 event-driven dispatch — IN PROGRESS.** Per-seat MCP-channel push replaces poll + send-keys + idle-scrape (Phase-1 delivery-swap; actuation-only; master-alone-merge preserved). **FRE-852/871/872 Done** (ADR + channel launch AC-2 proven live + channel delivery merged dormant; detail → HISTORY 2026-07-13). **FRE-875 (In Progress) = the seam:** Phase A drift-guard merged (PR #516, `topology.mode` single source of truth; inert) · **Phase B = the live per-seat cutover** (adr→build2→build1→master, prove AC-1/3/6 per seat, delete the idle-scrape last). **Ask-first deploy, parked for a rested master.** Phase-B prereqs on the ticket: orchestrator reload of the merged launcher · `SESHAT_CHANNEL_SECRET` provisioning before any flip · live-seat plugin-install + managed-settings (incl. `enableAllProjectMcpServers`). 873/874 canceled. ADR-0113/FRE-828 canceled (history).
