...
- 🚨 **ACTIVE DEPLOY-GATE (2026-06-28):** main now carries gateway code (FRE-369) that references the `upload_pending` column. **The NEXT `seshat-gateway` deploy of ANY kind — incl. the FRE-655 recall rollout — MUST first apply migration `docker/postgres/migrations/0012_upload_pending_column.sql` to prod Postgres** (`ALTER TABLE artifacts ADD COLUMN IF NOT EXISTS upload_pending BOOLEAN NOT NULL DEFAULT FALSE`; idempotent) and verify the column, or every artifact resolve/list/metadata/export + agent `artifact_read` will 500 ("column does not exist"). Owner chose **HOLD** on the FRE-369 deploy 2026-06-28 — FRE-369 stays In Progress (merged, dormant, live-verify pending). Clear this gate when 0012 is applied to prod.
