...
|--------|---------|-------------------|---------|--------------|
| build · Stream 1 · **Opus** _(flip to Opus for 724)_ | `/build 1` | **FRE-724** [O] _(recall v1 core SEAM — assemble arms→RRF→rerank into MEMORY_RECALL behind the flag; prove ADR-0104 AC-1/3/5/6 live. All deps met: 722✅ 723✅ 706✅)_ | CLEAR | — _(ADR-0104 Implemented on live proof; master gates the flag rollout)_ |
| build2 · Stream 2 · **Sonnet** | `/build 2` | **FRE-692** [S] _(per-attachment cloud/local override UI — PWA; carrier FRE-661 shipped)_ | **KEEP** _(direct follow-on to FRE-691 cost machinery)_ | FRE-693 [S] → vision cleanups 726[S]/735[S]/668[H]/742[H] → **master: deploy 691+692+693 + run FRE-669 seam = ADR-0101 DONE**. _(ADR-0102 doc chain 682–689 **owner-PAUSED** at Needs-Approval — do not build)_ |
| adr · **Opus** | `/adr` | **FRE-736** [O] _(stored-image re-vision ADR — **Approved**; extends ADR-0101 to stored artifacts via credentialed re-fetch)_ | CLEAR | 639↔ADR-0105 System-isolation reconciliation · pedagogical-reflection ADR (unfiled) |
<!-- STREAM-BOARD:END -->

**Context flag:** default **CLEAR** (fresh context per ticket). **KEEP** is set only when the NEXT ticket is a direct follow-on to what the stream just shipped (same files/substrate) — then run the command on the warm context, do **not** `/clear`. **Parked / owner-gated (not auto-dispatched):** FRE-713 (trigger-gated) · FRE-639 (held for the FRE-708 isolate-vs-gate decision) · FRE-704 (ES field-drop — master brings the raise-cap-vs-prune approach to owner). WIP = 1 live ticket per stream.
