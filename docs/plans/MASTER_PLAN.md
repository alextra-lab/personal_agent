...
- **ADR-0114 associative-memory study — BUILDING (priority-#2 decoupled side-study, owner-approved).** Chain FRE-838→843 (v0) + 853→856 (v1 arms D–G) all Approved. **FRE-838 (isolated substrate + frozen corpus) DONE** (PRs #468 + #471 + #473; verified live 2026-07-10 on the third attempt — the first two failed on study-Neo4j OOM then a 36%-relationship drop from a hardcoded allowlist, both caught at the master gate). The complete corpus (10,290 nodes / 34,301 rels, all 18 types incl. the associative RELATED_TO/USES/PART_OF/SIMILAR_TO graph) exports with prod zero-delta and a reproducible content_hash; completeness is now guaranteed by dynamic `db.labels()`/`db.relationshipTypes()` discovery + an exported==prod invariant test. **FRE-839 (schema + categorizer + writer) dispatched to build1**; 840→843 advance as the chain merges. Open finding: fold the arm-B (dedup-only) recall number into FRE-843's continuation gate.
