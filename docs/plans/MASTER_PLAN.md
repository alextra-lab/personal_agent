...
- **build1: ADR-0102 vision docs/PDF chain — BUILD COMPLETE.** All child leaves T1–T8 merged/inert-baking (T4/684 + T5/686 + T7/685 + T8/687 Awaiting Deploy; T6/688 Done test-only). **build1 idle** (no further vision leaves; seam is master-run). Only the **master-run seam T9/689 (AC-SEAM live smoke) remains** — it is unblocked and is the go-live event. **Seam deploy = gateway rebuild + PWA rebuild together** (T8/687 is a PWA control that must ship with its backend, not before); AC-SEAM fires paid live turns (leg 4 = a cloud document with real cost) → **owner-gated deploy + smoke** (gateway rebuild pre-authorized "if necessary", but confirm timing + the live-turn OK). **build2: FRE-884** — retire the orphaned ADR-0086 artifact-decomposition path (enabled via `.env` but feeds nothing; flag-off + delete + ADR-0086→retired; deploy ask-first). **adr/explore: multi-model-harness deliberation** — Stream-1 pipeline-coherence/orphan-cleanup (FRE-884 is its retirement half; routing-collapse + resolve-or-remove flag gate = its ADR), Stream-2 model-selection picker (ADR-0118 Phase-1 seed). Owner-hubbed, post-vision, **NOT master-steered**. Parked-Approved: FRE-858 keep-bugs · **FRE-876** · **FRE-867** · **FRE-807**.
...
- **ADR-0116 event-driven dispatch — IN PROGRESS (Phase B cutover DONE 2026-07-14).** Per-seat MCP-channel push replaces poll + send-keys + idle-scrape (Phase-1 delivery-swap; actuation-only; master-alone-merge preserved). **FRE-852/871/872 Done.** **FRE-875 (In Progress) = the seam:** Phase A drift-guard merged (PR #516) · **Phase B — all 3 worker seats (adr/build2/build1) cut over to channel-mode** (PRs #519 merged; box setup done: plugin install + managed-settings + shared secret; each seat verified live — channel registers, authorized POST → 200 fires a turn, wrong secret → 403). Master stays send-keys (no topology entry; scrape-free). **Live since 2026-07-14.** **Remaining before the seam closes:** (1) **seat-secret durability** — the seat-side secret lives in the tmux global env, lost on a tmux/box restart (degrades safely to send-keys) → persist it (small launcher change); (2) **scrape deletion** — the ADR's final AC (the scrape is still the channel-down fallback idle-gate + the master context-pressure nudge's gate; re-home both, not just delete). 873/874 canceled. ADR-0113/FRE-828 canceled (history).
...
**Sequencing rationale (current):** build1 = ADR-0102 vision chain — BUILD COMPLETE (T1–T8 all merged/inert-baking → seam 689 master-run, the go-live). build1 idle until owner reassigns.
build2 = ADR-0116 channel track (FRE-872 wiring → FRE-875 cutover, ask-first). Both parallel. After each stream
frees: build2 → the channel cutover, then the parked FRE-858 recall keep-bugs + FRE-876. FRE-739 merged (Awaiting
Deploy, ADR-0107 seam owner — assembled verify needs FRE-740 + a live non-owner request). FRE-717 alone held at
Awaiting Deploy for organic outcome input.
...
1. **Agentic Vision** — ADR-0101 **Implemented** (images live). **ADR-0102 documents/PDF — BUILD COMPLETE, go-live pending** (owner-directed): T1–T8 all merged/inert-baking; seam FRE-689 (master-run deploy + AC-SEAM live smoke) is the go-live, owner-gated. **ADR-0108 stored-image reprocess — lower priority, HELD** (ADR still Proposed; owner accepts via `/adr` before dispatch).
...
| ADR | Title | Status |
|-----|-------|--------|
| **0116** | Event-driven dispatch actuation (MCP channels) | **Accepted, IN PROGRESS.** FRE-852 (ADR) + FRE-871 (channel + AC-2 proven live) Done; FRE-872 (consolidated wiring) building; FRE-875 cutover (ask-first) pending. Supersedes ADR-0110's transport half. |
