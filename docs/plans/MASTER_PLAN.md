...
> **Last updated**: 2026-05-14 — FRE-363 operator stanza bug merged (PR #51) · FRE-326 re-eval complete: host-resource gates retained as deployment-conditional safety valve for local-inference mode (no code change) · FRE-315 mermaid rendering + validated authoring skill shipped (PR #53).
...
| Item | Date | Summary |
|------|------|---------|
| **FRE-315: Mermaid rendering + validated authoring skill (PR #53)** | 2026-05-14 | Two halves shipped together. **PWA half**: `seshat-pwa/src/components/MermaidBlock.tsx` integrates mermaid.js v11 into `MarkdownContent.tsx` — lazy chunk-split via dynamic import (mermaid never appears in the initial bundle, confirmed across 5 lazy chunks), custom `theme: 'base'` + `themeVariables` matching the seshat slate/blue-500 palette, figure/source toggle with 150ms cross-fade, graceful syntax-highlighted fallback with a rose-tinted error annotation on parse failure. SVG inserted via `DOMParser` + `replaceChildren` under `securityLevel: 'strict'` — no string-injection HTML patterns. **Skill half**: `docs/skills/mermaid-diagrams.md` with full frontmatter (44 keywords, nudge directive per ADR-0067), 9 syntax invariants for v11, six worked examples (flowchart, sequence, state, class, C4, ER), and an **automated validation pattern** using `mmdc` in-container before the diagram reaches the user. `Dockerfile.gateway` gains `nodejs`, `chromium`, `fonts-liberation`, `@mermaid-js/mermaid-cli@^11` with an `/usr/local/bin/mmdc` wrapper that injects `--no-sandbox` via puppeteer config (Chromium-as-root requirement); `mmdc` added to bash auto-approve in `config/governance/tools.yaml`. Image grew ~800MB — accepted cost for closing the LLM-authored-mermaid failure loop. Discovered+documented mmdc quirk: exits 0 even on parse error, so the skill validates by output-file presence (`test -s /tmp/m.svg`) rather than `$?`. Linear FRE-315 description expanded during planning to add the skill scope. |
