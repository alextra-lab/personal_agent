...
- **FRE-523** ⏳ OPEN — deployed (PR #208, `46a68c1`). **2026-06-13 eval pass run (`fre523-verify-01`, 18 cases):** capture-layer AC **pass** — 24 `eval_mode=true` primary + 4 sub-agent captures + 24 reflections written (was 0); provenance + eval/real isolation confirmed (real "cars" turn correctly `eval_mode=false`). **KG-half (AC-1 KG write + AC-3 cross-run recall) BLOCKED on 🔴 FRE-560** — consolidation never fires, so captures don't reach Neo4j. Resume 523's KG verification + pass 2 once FRE-560 ships. Original remaining step (still valid post-FRE-560):
...
1. FRE-552 ✅ (PR #209) → 2. FRE-547 ✅ (PR #210) → 3. FRE-543 ✅ ILM/retention (PR #212, deployed) → **4. FRE-544 dyn-field bound ← next** → 5. FRE-546 cost-cache Kibana import fix → 6. FRE-550 joinability breakdown panels → 7. FRE-477 discovery batching (Turn Cost) → 8. FRE-487 govern model file-reads (Turn Cost).

**Worktree B — Topology/eval/ledger + Artifact** (build-ready only — ADR-0088/0090 accepted; topology projector · route-trace ledger · eval harness · artifact_tools):
1. FRE-523 ✅ eval-mode memory bug (PR #208; capture-AC ✅, KG-AC blocked on FRE-560) → 2. FRE-517 ✅ per-topology rows (PR #211) → 3. FRE-548 ✅ topology→ES projection (PR #213, deployed) → **4. FRE-545 project ledger routing fields → ES ← next** *(reuses 548 pattern)* *(Telemetry ticket; here to avoid colliding with 517 on topology code)* → 4. **FRE-545 project route-trace-ledger routing fields → ES** *(re-anchored 2026-06-10: ADR-0088 ledger projection, NOT a new ADR-0082 event; reuses the 548 pattern)* → 5. FRE-522 eval⇄PWA → 6. FRE-541 conversation driver (unblocks 453; Tier-1 but no new ADR — amends taxonomy spec) → 7. FRE-551 artifact E2E extend.

**ADR session (NOT a build worktree — needs an ADR/research doc before any implementation):**
- **FRE-507** — *rescope vs 513 first* (513 already publishes live cost at the boundary, D3 — the rescope may shrink or close it). If it survives → ADR (turn_status partial-merge contract, idempotency, consumer lifecycle) → impl ticket. **Was wrongly slotted into a build worktree.**
- **FRE-497** (Turn Reliability close) — ADR: self-correcting-gate contract (retry budget, feedback shape, cost accounting, eligibility predicate, telemetry) → impl tickets.
- **FRE-474** (Turn Reliability close) — cross-provider cache research doc (`docs/research/`), cited to current provider docs.

*Turn-project closure routing:* Turn Cost closes via 477+487 (build worktree A); Turn Reliability closes via 497+474 (**adr session**, not a build worktree).
