...
one gate, and the chain advances when master runs advance-dispatch at the merge. A ten-ticket chain is
ten cold starts, ten plans, ten gates, ten merges.
...
| ADR-0130 (PR #774, open) and its three tickets | **Sub-agent fan-out cost on a real chain — never measured** |
| ADR-0129 + its ten-ticket chain, read as the specimen | Transcripts. No claim here about what any session believed |
...
Two facts are verified rather than asserted: ADR-0129 is 376 lines, and its chain is ten tickets
(FRE-1064, 1065, 1067, 1069, 1070, 1071, 1072, 1073, plus FRE-1066 and FRE-1068). Everything about what
those ten cost to deliver under either model is unmeasured.
...
The obvious argument is amortization: ten sessions each cold-read the same 376-line ADR and the same
...
**(b) One PR for the chain.** Master gates once, over a diff that is ten tickets wide.
...
- Master's cheapest lever disappears. Today a bad call in child 2 bounces at child 2, before children
  3–10 are written against it.
- The rollback unit grows with the chain. Child 4 of 10 fails its criteria — bounce all ten, or land
  nine and revert one? Neither answer is good.
...
proposal is that label generalised to N = 10 with sub-agent fan-out. That reframing matters: the first
...
makes an unmeasured fan-out on a ten-ticket chain a poor first move.
...
**Which chain not to use.** ADR-0129's is the obvious candidate — ten tickets, already filed — and the
...
5. **Land PR #774 unchanged.** ADR-0130 is correct under today's model. Amending an open ADR to
   accommodate an untested proposal would be worse than amending it later with the reasoning recorded.
...
- **The self-marking analysis assumes ADR-0130 merges as written.** PR #774 is open. If D2 changes at
  the gate, §3.1 and §3.2 need re-reading.
