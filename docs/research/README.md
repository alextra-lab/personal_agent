...
- **`2026-07-27-feedback-layer-and-evidence-contract-audit.md`** — Audit of the feedback-analysis layer (Captain's Log reflection, Insights, promotion, capture) and the evidence a verification layer would need. Evidence base for **ADR-0125**. Headline: the project has pursued **two undistinguished quality dimensions** — harness health versus output quality — and the capture layer proves it, recording `memory_context_used: bool` and a count while the *only* per-turn entity identities persisted are the ones compaction **dropped**: the system records what it discarded, not what it relied on. Also: ADR-0105's convergence **shipped 7 of 8** and the loop has never run end-to-end (FRE-717 awaiting deploy since 2026-07-08); the reflection producer is **correctly scoped** for dimension 1 (an earlier "it can't see the answer" finding is **retracted** — recorded deliberately); the 200-character clip is a **nine-site idiom** whose worst instance is live in context assembly and strictly worse than the summarizer already rejected; provenance and supersession exist while **usage does not**; and the verification oracle's feasibility bounds (evidence-relative, three-valued with `UNVERIFIABLE` first-class, sampled) plus its eight required capture inputs. Carries an explicit **Limitations** section — tool-payload retention, `corroboration_count` populators, and the post-ADR-0115 staleness of the `class=None` figures are all unverified
...
- **`2026-06-09-fre-538-monitors-dashboard.md`** — C3 monitors dashboard (joinability ADR-0074 + SLM-health ADR-0083): surfaces two monitor index families that had zero visualizations. The decisive finding is the **SLM-health mapping straddle** — `status`/`error` are `text`+`.keyword` in historical dynamic-mapped indices but bare `keyword`/`text` in the post-FRE-534 template index, so no single field name aggregates across all data; `reachable` (boolean) is the join-safe availability signal (`true 1598 / false 240`, reconciles exactly). Aggregations pinned to straddle-safe fields; `status`/`error` surfaced via saved-search `_source`; nested per-substrate orphan breakdown deferred (legacy aggs can't aggregate `nested`). `outcome:(red OR yellow)` == the ADR-0074 orphans>0 halt condition
