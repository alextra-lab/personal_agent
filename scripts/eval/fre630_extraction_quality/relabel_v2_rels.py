...
Like ``relabel_v2_types.py`` this is a one-off research/labeling script: raters
dispatch through ``LiteLLMClient`` constructed directly (not via the factory's
``get_llm_client_for_key`` — the "full" gpt-5.4 rater has no catalog entry, and
``LiteLLMClient``'s own constructor docstring sanctions direct construction for
a deployment genuinely outside the catalog), which puts every call behind the
egress guard and the cost gate (``budget_role="study"``, the same one-off
research lane ``scripts/study/categorizer.py`` uses) — documented in
docs/superpowers/plans/2026-07-04-fre-773-relationship-v2-validation.md and
ADR-0141 (FRE-1367).
