...
class DocsLinkChecker:
    def __init__(self, docs_root: Path, check_external: bool = True, timeout: int = 5):
        ...
        self.link_map: Dict[Path, List[Tuple[str, str, str]]] = defaultdict(list)  # file -> [(link_text, link_url, link_type)]
    ...
    def check_internal_link(self, file_path: Path, link_url: str) -> Tuple[bool, str]:
        ...
        if url_part.startswith('./'):
            ...
            if not target_path.is_relative_to(self.docs_root):
                ...
                return False, f"Link points outside docs directory"
        ...
        if anchor:
            ...
            if not anchor_valid:
                return False, f"Anchor '#{anchor}' not found in {target_file.relative_to(self.docs_root)}"
        ...
        for file_path, links in self.link_map.items():
            for link_text, link_url, link_type in links:
                if link_type == 'internal':
                    ...
                    if not any(indicator in link_url for indicator in ['./', '../', '/', '.md']) and len(link_url) <= 15:
                        continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify documentation structure and hyperlinks"
    )
    parser.add_argument(
        '--docs-dir',
        type=str,
