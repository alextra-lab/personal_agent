...
function ListSkeleton() {
  return (
    <div className="px-4 py-6 space-y-2">
      {[0, 1, 2, 3].map((i) => (
        <div key={i} className="h-10 rounded-lg bg-surface/60 animate-pulse" />
      ))}
    </div>
  );
}
...
export function ObserveView() {
  ...
  return (
    <div className="min-h-full bg-bg text-ink flex flex-col">
      {/* Header */}
      <header
        className="flex items-center gap-3 px-4 border-b border-line bg-bg/80 backdrop-blur-sm flex-shrink-0"
      ...
      >
        ...
        <h1 className="text-sm font-semibold text-ink">Observe</h1>
      </header>
          ...
          <>
              ...
              <div className="rounded-xl border border-line divide-y divide-line overflow-hidden">
                {roleEntries.map(([role, entry]) => (
                  <div key={role} className="flex items-center gap-3 px-3 py-2.5 text-sm">
                    <span
                      // Outlined, not filled, for both states — a tinted
                      // bg-accent/bg-line fill under colored/muted text
                      // measured under WCAG AA (fill + text pull toward the
                      // same tone); an outline reads clearly without that
                      // trap (FRE-1265 e2e contrast check).
                      className={`text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded flex-shrink-0 w-14 text-center border ${
                        entry.open
                    ...
                    <span className="font-medium text-ink w-36 flex-shrink-0 truncate">{role}</span>
                    {entry.resolved ? (
                ...
                {roleEntries.length === 0 && (
                  <p className="px-3 py-4 text-xs text-ink-muted">No roles configured.</p>
                )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="rounded-xl border border-line divide-y divide-line overflow-hidden">
                  ...
                  <div key={p.key} className="flex items-center gap-3 px-3 py-2.5 text-sm">
                    <span
                      className={`w-2 h-2 rounded-full flex-shrink-0 ${
                        p.available ? 'bg-emerald-400' : 'bg-line'
                      }`}
                ...
                {providers.length === 0 && (
                  <p className="px-3 py-4 text-xs text-ink-muted">No providers configured.</p>
                )}
              </div>
            </section>
          </>
