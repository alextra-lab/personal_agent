"""Persist :class:`CacheErosionResultDoc` instances to Elasticsearch (ADR-0078 / FRE-1189).

Index name: ``<prefix>-YYYY-MM`` (monthly, FRE-543 convention). Document id = a fresh
UUID so each hourly run accumulates as a distinct document.
...
from personal_agent.observability.cache_erosion.result import CacheErosionResultDoc
...
def index_name_for(doc: CacheErosionResultDoc, *, prefix: str) -> str:
    """Compute the monthly index name for a result doc.

    Args:
        doc: The result doc to store.
        prefix: Elasticsearch index prefix (e.g. ``"agent-monitors-cache-erosion"``).
...
async def write_result(
    ...
    *,
    ...
    Args:
        ...
        prefix: Index prefix from settings (e.g. ``settings.cache_erosion_probe_index_prefix``).
