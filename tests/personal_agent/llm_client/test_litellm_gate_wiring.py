...
async def real_gate() -> AsyncIterator[CostGate]:
    ...
    Unlike ``gate_for_role`` (a synthetic per-test role + config), this
    exercises the actually-declared ``artifact_builder`` / ``main_inference``
    policies so a regression that removes either role's YAML declaration
    fails this test too (AC-2b, ADR-0118 T1 / FRE-879).
    """
...
async def test_artifact_builder_lane_isolated_from_main_inference(
    ...
    """AC-2b (ADR-0118 T1, FRE-879): a real artifact-builder call debits its own
    lane and leaves main_inference untouched — proving the role was truly
    extracted, not just renamed (ADR-0118 Decision §1 failure mode).
    """
    ...
    builder_key = resolve_role_model_key("artifact_builder")
    ...
    assert model_def.id  # sanity: the matrix resolved to a real model definition
