...
@pytest.mark.asyncio
async def test_vision_escalation_labels_respond_call_role_vision(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """FRE-1037: a vision-escalated call must report role=VISION to .respond(), not
    ...
    trace_ctx = TraceContext.new_trace()
    mock_escalated_client = MagicMock(spec=LiteLLMClient)
    mock_escalated_client.respond = AsyncMock(return_value=_make_minimal_response())
    mock_session = MagicMock()
    mock_session.add_message = AsyncMock()
    mock_session.get_messages = AsyncMock(return_value=[])
    ...
    with (
        ...
        patch(
            ...
            return_value=mock_escalated_client,
        ),
        patch(
            "personal_agent.orchestrator.executor.get_default_registry",
            return_value=MagicMock(get_tool_definitions_for_llm=MagicMock(return_value=[])),
