"""Tests for async background compression manager."""
...
class TestGetSummary:
    ...
    async def test_returns_summary_from_completed_task(self) -> None:
        async def mock_compress(msgs: list, trace_id: str) -> str:
            return "compressed summary"

        with patch(
            "personal_agent.orchestrator.compression_manager.compress_turns",
            side_effect=mock_compress,
        ):
            task = asyncio.create_task(
                compression_manager._run_compression("s1", [_msg("user", 100)], "t1")
            )
...
class TestMaybeTriggerCompression:
    ...
    async def test_triggers_above_threshold(self) -> None:
        ...
        mock_compress = AsyncMock(return_value="summary")
        with (
        ...
        assert compression_manager._summaries["s1"] == "summary"
